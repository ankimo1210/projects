#!/usr/bin/env python3
"""Generate BSM Chapter 15 Jupyter Notebook."""
...
# ============================================================
# Section 0: Title and Learning Objectives
# ============================================================
cells.append(md(r"""# Chapter 15 — The Black-Scholes-Merton Model
...
cells.append(code(r"""# ===== 2-1. GBM シミュレーション =====
...
def implied_vol_newton(market_price, S, K, T, r, option_type="call",
    ...
    for _ in range(max_iter):
        ...
        sigma = max(sigma, 1e-6)  # 負にならないよう制約
...
cells.append(code(r"""# --- 3-1. GBM パスのシミュレーション ---
S0 = 100.0
mu = 0.10
sigma = 0.20
T = 1.0
...
cells.append(code(r"""# --- 3-2. S_T の分布 ---
ST = S[:, -1]
...
cells.append(md(r"""### 確認ポイント (Section 3)
...
cells.append(code(r"""# --- 4-1. サンプル価格系列の作成 ---
n_days = 504  # 2年分
...
axes[2].axhline(0.25, color="black", linestyle="--", linewidth=1, label="True σ=0.25")
...
cells.append(md(r"""### 確認ポイント (Section 4)

1. 窓長を変えるとローリング・ボラティリティの推定値がどう変わるか確認しましたか？
2. 全期間の推定値は真の $\sigma=0.25$ にどれくらい近いですか？
3. 短い窓長でのローリング推定値のブレを見て、**モデルの仮定（σ一定）と現実の乖離**を考えてみましょう。"""))

# ============================================================
# Section 5: Delta hedging & BSM PDE
# ============================================================
cells.append(md(r"""---
...
cells.append(code(r"""# --- 5-1. デルタヘッジの数値デモ ---
...
cells.append(code(r"""# --- 5-2. Option payoff 曲線 ---
...
cells.append(md(r"""### 確認ポイント (Section 5)
...
cells.append(code(r"""# --- 6-1. モンテカルロによる European Call 価格 ---
...
cells.append(code(r"""# --- 6-2. mu を変えたときの比較 ---
...
for i, mu_i in enumerate(mu_values):
    ...
    axes[0].hist(payoff_i[payoff_i > 0], bins=60, density=True, alpha=0.35,
                 label=f"μ={mu_i:.2f}", color=colors_cycle[i % len(colors_cycle)])
...
axes[0].set_title("Payoff 分布 (μ を変えた場合)")
axes[0].set_xlabel("Payoff max(S_T - K, 0)")
axes[0].legend(fontsize=8)

# (b) 各 mu での「価格」
df_mc = pd.DataFrame(mc_prices, columns=["mu", "disc_by_mu", "disc_by_r"])
axes[1].bar(np.arange(len(mu_values)) - 0.15, df_mc["disc_by_mu"], 0.3, label="Discount by μ")
...
axes[1].set_xticklabels([f"μ={m:.2f}" for m in mu_values])
...
cells.append(md(r"""### 確認ポイント (Section 6)
...
# ============================================================
# Section 7: BSM formula implementation
# ============================================================
cells.append(md(r"""---
## 7. §15.8 BSM 公式の実装
...
cells.append(code(r"""# --- 7-1. サンプルパラメータで価格計算 ---
...
cells.append(code(r"""# --- 7-2. Option price vs Spot ---
...
cells.append(code(r"""# --- 7-3. Option price vs Volatility ---
...
cells.append(code(r"""# --- 7-4. Option price vs Maturity ---
...
cells.append(md(r"""### 確認ポイント (Section 7)
...
cells.append(code(r"""# --- 8-1. 標準正規 pdf / cdf の描画 ---
...
cells.append(md(r"""### 確認ポイント (Section 8)
...
cells.append(code(r"""# --- 9-2. 希薄化前後の 1株価値 比較図 ---
...
cells.append(md(r"""### 確認ポイント (Section 9)
...
cells.append(code(r"""# --- 10-1. Option price vs volatility (root finding の可視化) ---
...
cells.append(code(r"""# --- 10-2. Bisection vs Newton 収束比較 ---
...
ax.set_ylabel("$\\sigma$ estimate")
ax.legend()
plt.tight_layout()
plt.show()"""))

cells.append(code(r"""# --- 10-3. Strike 別 IV (Volatility Smile の入口) ---
...
print("→ BSM では σ 一定を仮定するが、市場では Strike ごとに IV が異なる (Volatility Smile)")"""))

cells.append(md(r"""### 確認ポイント (Section 10)
...
cells.append(code(r"""# --- 11-1. q を変えたときの Call / Put 価格 ---
...
cells.append(code(r"""# --- 11-2. q=0 vs q>0 の比較 ---

S_arr2 = np.linspace(60, 140, 200)
q_val = 0.03

C_q0 = bsm_call_price(S_arr2, K_ex, T_ex, r_ex, sigma_ex)  # q=0
C_q3 = np.array([bsm_call_with_dividend_yield(s, K_ex, T_ex, r_ex, sigma_ex, q_val) for s in S_arr2])
P_q0 = bsm_put_price(S_arr2, K_ex, T_ex, r_ex, sigma_ex)
P_q3 = np.array([bsm_put_with_dividend_yield(s, K_ex, T_ex, r_ex, sigma_ex, q_val) for s in S_arr2])
...
cells.append(md(r"""### 確認ポイント (Section 11)
...
# ============================================================
# Section 12: Appendix 15A numerical check
# ============================================================
cells.append(md(r"""---
## 12. 付録 15A の数値確認
...
cells.append(code(r"""# --- 12-1. Monte Carlo vs BSM ---
...
cells.append(code(r"""# --- 12-2. 収束グラフ ---
...
cells.append(md(r"""### 確認ポイント (Section 12)
...
# Q1
cells.append(md(r"""### 問題 1: パラメータ変更時の価格変化
...
ax.axhline(0.25, color="black", linestyle="--", label="True σ=0.25")
...
print("→ 窓長が短いほどノイジーだが局所変動を捉やすい。")
print("  窓長が長いほど滑らかだが変化への反応が遅い。")"""))

# Q5
cells.append(md(r"""### 問題 5: デルタヘッジ誤差の確認
...
# Fix cell sources: each line should end with \n except the last
