...
from ui.unit_price import (
    ...
    yen_per_sqm_to_tsubo,
    yen_per_sqm_to_tsubo_man,
    yen_to_man,
)
from ui.table import render_html_table, gap_bar, price_man, muted, year_num
...
def render_map_tab(df: Optional[pd.DataFrame], filters: dict, conn=None) -> None:
    ...
    if df is None or df.empty:
        st.info("データがありません。左サイドバーで年度を選択するか、管理タブからデータを同期してください。")
        return
    ...
    # ── customdata の準備 ─────────────────────────────────────
    custom_cols = ["point_id", "standard_land_number", "location_text",
                   "city_name", "yoy_change_pct", "use_category_name", "area_sqm",
                   "_yoy_str", "_price_sqm_man_str", "_price_tsubo_str"]
    for c in ["point_id", "standard_land_number", "location_text",
              "city_name", "yoy_change_pct", "use_category_name", "area_sqm"]:
    ...
    if display_mode == "絶対価格 (万円/m²)":
        ...
        plot_df["_price_score"] = _compute_category_score(plot_df, "price_man_per_sqm", "use_category_name")
        ...
        _apply_hover(fig, color_label="価格", color_format="{:,.1f}万円/m²", color_col="price_man_per_sqm", df=plot_df)
        ...
        _apply_hover(fig, color_label="前年比", color_format="{:+.1f}%", color_col="yoy_clamped", df=yoy_df)
        plot_df = yoy_df
...
def _apply_hover(fig, color_label: str, color_format: str, color_col: str, df: pd.DataFrame) -> None:
    """読みやすいホバーテンプレートを設定する。"""
    if color_label == "価格":
        tmpl = (
            _HOVER_BASE
...
def _render_selected_point(event, conn) -> None:
    ...
    fig.update_layout(
        ...
        yaxis2=dict(title="前年比 (%)", overlaying="y", side="right",
                    ticksuffix="%", tickformat=".1f", showgrid=False),
    ...
    with st.expander("📋 年次データ一覧", expanded=False):
        show_cols = [c for c in [
            ...
            "yoy_change_pct", "use_category_name", "zoning_name",
        ] if c in history.columns]
        display_history = add_tsubo_price_column(history[show_cols], "price_yen_per_sqm", "price_yen_per_tsubo")
        display_history = add_tsubo_price_column(display_history, "last_year_price_yen_per_sqm", "last_year_price_yen_per_tsubo")
        display_history = convert_yen_columns_to_man(display_history, ["price_yen_per_sqm", "last_year_price_yen_per_sqm"])
        render_html_table(display_history.sort_values("year", ascending=False), [
