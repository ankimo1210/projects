...
def render_admin_tab(conn) -> None:
    ...
    sync_year = col1.number_input("同期年度", value=2026, min_value=2000, max_value=2030, step=1, key="sync_year")
    ...
    batch_year_start = b2.number_input("開始年", value=2010, min_value=2000, max_value=2030, step=1, key="batch_year_start")
    batch_year_end = b3.number_input("終了年", value=2026, min_value=2000, max_value=2030, step=1, key="batch_year_end")
    ...
    if st.button("▶ バッチ同期開始", type="primary", key="batch_sync_btn"):
        ...
        if not years_to_sync:
            ...
            label = f"都道府県 {pref}" if pref else "全国"
            st.info(f"{label} の {batch_year_start}〜{batch_year_end} 年を同期します（{len(years_to_sync)} 年分）")
    ...
    trade_year = t2.number_input("年", value=2024, min_value=2000, max_value=2030, step=1, key="trade_sync_year")
    ...
    geo_limit = st.number_input("処理件数上限（ユニーク地点数）", min_value=10, max_value=5000, value=200, step=50, key="geo_limit")
    geo_sleep = st.number_input("API間隔（秒）", min_value=0.1, max_value=2.0, value=0.3, step=0.1, key="geo_sleep")
    ...
    if st.button("▶ ジオコーディング開始", type="primary", key="geocode_btn"):
        ...
        with st.spinner("ジオコーディング実行中..."):
            result = run_geocoding(limit=int(geo_limit), sleep_sec=float(geo_sleep), progress_cb=_cb)
    ...
    r1, r2 = st.columns(2)
    rec_limit = r1.number_input("再計算件数", min_value=1, max_value=5000, value=100, step=50, key="recompute_limit")
    rec_stale = r2.number_input("再計算TTL(日)", min_value=0, max_value=365, value=7, step=1, key="recompute_stale")
    ...
    market_year = m2.number_input("対象年 (0=全体)", min_value=0, max_value=2030, value=0, step=1, key="market_feature_year")
    market_limit = m3.number_input("件数上限", min_value=1, max_value=20000, value=500, step=100, key="market_feature_limit")
    ...
    model_year = st.number_input("学習年 (0=全体)", min_value=0, max_value=2030, value=0, step=1, key="model_train_year")
    model_limit = st.number_input("学習件数上限", min_value=100, max_value=50000, value=10000, step=500, key="model_train_limit")
    ...
    if st.button("▶ 線形モデルを学習", type="primary", key="train_model_btn"):
        with st.spinner("学習中..."):
            ...
            if df_train.empty:
                ...
                st.success(f"保存完了: {path.name} / rows={model['row_count']} / rmse_log={model['rmse_log']:.4f}")
    ...
    max_pages = sr3.number_input("最大ページ数", min_value=1, max_value=50, value=3, step=1, key="search_import_pages")
    max_listings = sr4.number_input("最大物件数", min_value=1, max_value=500, value=60, step=10, key="search_import_listings")
    search_sleep = sr5.number_input("取込間隔(秒)", min_value=0.0, max_value=10.0, value=1.5, step=0.5, key="search_import_sleep")
    ...
    if st.button("▶ 検索結果URLから収集して取込", type="primary", key="search_import_btn"):
        if not search_url.strip():
            ...
            def _progress_cb(payload: dict[str, object]) -> None:
                ...
                elapsed_str = f" | 経過 {_fmt_elapsed(batch_elapsed)}" if batch_elapsed is not None else ""
                if stage == "collect":
                    ...
                    if event == "page_start":
                        ...
                        log_lines.append(f"[収集] page {page_no}: +{new_count} 件 / total {collected_urls_local} 件")
                    ...
                    url_elapsed_str = f" ({_fmt_elapsed(url_elapsed)})" if url_elapsed is not None else ""
                    if event == "phase_start":
                        status.caption(f"取込開始: 0/{total}")
                    elif event == "start":
                        status.caption(f"取込中: 成功 {done} / 失敗 {failed} / total {total}{elapsed_str}")
                        ...
                        log_lines.append(f"[取込失敗]{url_elapsed_str} {payload.get('url')} — {error_detail}")
                        log_box.code("\n".join(log_lines[-12:]), language="")
                elif stage == "features":
                    total = int(payload.get("total") or 0)
                    done = int(payload.get("done") or 0)
    ...
    if not recent_jobs.empty:
        cols = [
            c for c in [
                "started_at", "source", "region_label", "status",
                "collected_pages", "collected_urls", "imported_done",
                "imported_failed", "feature_done", "feature_failed", "search_url",
            ]
    ...
    region_label = st.text_input("地域ラベル", placeholder="例: 新宿区一棟アパート", key="listing_import_region")
    ...
    with col_c:
        ...
        if st.button("⚠ 年度データ削除", type="secondary"):
            if del_year > 0:
                ...
                if cnt > 0:
                    conn.execute(
                        "DELETE FROM land_prices_public_notice WHERE year=?", [del_year]
                    )
    ...
    for d, label in [(RAW_DIR, "raw"), (PROCESSED_DIR, "processed")]:
        for p in sorted(d.glob("*")):
            if p.is_file():
                ...
                files.append({"ディレクトリ": label, "ファイル名": p.name, "サイズ(KB)": f"{size_kb:.1f}"})
    if files:
