...
| 2 | **WP2: Blueprint 品質ラン** 🔄 | B（並行） | 15k iters 計測。初回(06-12 K_r/K_t=32)は同時開発負荷で **OOM**（dense 31.98GB>48GB、結果なし）。**06-13 再実行 K_r=24/K_t=16 dense 23.95GB 単独**。所見: dense を支配するのは K_r（K_t は無関係） | 計算 ~7h | — |
