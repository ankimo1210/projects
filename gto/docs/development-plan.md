...
| 2 | **WP2: Blueprint 品質ラン** ✅ | B | 06-13 完了。K_r=24/K_t=16/15k → **expl 0.151bb**（≈06-09 の K32 0.147、残差は全て SB 側、BB 0.031）。**G2→基盤投資先行**（<0.1bb 未達・river 粒度飽和）。先に 30k iters プローブで収束 vs 抽象化を切分け | 計算 ~5.5h | **G2 決定済** |
| 3 | **WP3: E1 改訂 + 実装** | A | ゲーティング表改訂 → 公開デプロイ + 認証 | 0.5 + 2–3 日 | G1: M1b 完了 ✅ |
| 4 | **WP4: M2** ✅ | A | 6max v1 + Tournament-HU + 移行（**完了 2026-06-12**。CPU並列化実験のみ未着手 — G3 入力として残） | 4–6 日 | — |
...
| **G2** ✅ | WP2 30k プローブ後（06-14 確定） | **→ 収束不足が支配的・抽象化の限界ではない**。15k→30k で 0.151→0.106（下げ止まらず）、BB 完全収束、残差は SB のみ。収束則 ~1/√T = public-chance サンプリングの分散起因。**M3 最優先=分散削減**（ターン enumerate/マルチサンプル）で ~1/T 回復。f32/ボードバケット/ディスクは M 拡大用の別軸 |
