...
def _build_dfs(results: list[dict]) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    spots_rows = [{
    ...
    combo_rows = [
        ...
        for r in results for cs in r["combo_strats"]
    ]
    ...
    for r in results:
        ...
        report_rows.append({
            "position": r["position"], "opponent": r["opponent"],
            "stack_bb": r["stack_bb"], "board": r["board"], "texture": r["texture"],
    ...
    agg_df = pd.DataFrame(agg_rows) if agg_rows else pd.DataFrame(
        columns=["spot_id", "action", "freq"])

    if combo_rows:
        spot_ids, card_as, card_bs, actions, freqs = zip(*combo_rows)
    ...
    reports_df = pd.DataFrame(report_rows) if report_rows else pd.DataFrame(
        columns=["position", "opponent", "stack_bb", "board", "texture",
                 "check_freq", "bet33_freq", "bet75_freq", "bet100_freq"])
...
def run_batch(
    ...
    for pos, opp, stack, flops in groups:
        ...
        for i in range(0, len(flops), batch_size):
            ...
            spots_input = [
                {"board": list(f), "pot_bb": pot, "effective_stack_bb": eff}
                for f in chunk
            ]
            ...
            batch_results = []
            for flop, result in zip(chunk, gpu_results):
                board_str = "".join(flop)
...
def migrate_duckdb() -> None:
    ...
    spots_df   = con.execute("SELECT spot_id, position, opponent, stack_bb, pot_bb, board, street, iterations, exploitability FROM spots").df()
    agg_df     = con.execute("SELECT spot_id, action, freq FROM aggregate_strategies").df()
    combos_df  = con.execute("SELECT spot_id, card_a, card_b, action, freq FROM combo_strategies").df()
    reports_df = con.execute("SELECT position, opponent, stack_bb, board, texture, check_freq, bet33_freq, bet75_freq, bet100_freq FROM flop_reports").df()
    con.close()
...
if __name__ == "__main__":
    ...
    p.add_argument("--rebuild-cache",  action="store_true", help="Rebuild aggregate JSON cache for frontend")
