...
- [x] ブループリント品質ラン（WP2、G2 の入力）完了 — 2026-06-13。
  ...
  - 30k プローブ（同 K、2026-06-14 完了、~13.5h 単独・ポーズなし）: **expl
    0.106 bb**（BR_sb 0.224 / **BR_bb -0.011 = BB 完全収束**、SB root
    fold20/call61/raise19%）。結果: `_data/gto/hu/blueprint_probe30k_20260614/`。
  - **G2 判定（確定、30k プローブで訂正）→ 収束不足が支配的・抽象化の限界では
    ない**。15k→30k で expl 0.151→0.106（下げ止まらず）、BB は不可剝離、残差は
    SB のみで継続低下。**K_r=24 抽象化はまだ拘束条件ではない**ので f32/ボード
    バケッティング/ディスクバックの基盤投資を expl 削減目的で先行する必要は無い。
  - **収束則 expl ∝ T^-0.51 ≈ 1/√T**（DCFR を厳密木なら本来 ~1/T）。この遅さは
    **public-chance サンプリング（ターン sample）の分散**が原因。iters ブルート
    では 0.05 到達 ~135k iters(~2.5日)で非現実的。**→ M3 の最優先は分散削減**
    （ターン enumerate or マルチサンプル/external sampling）で ~1/T を回復させる。
    基盤投資（f32/ボードバケット/ディスク）は **M 拡大（代表 flop 数）用の別軸**
    で、現状の expl ボトルネックではない。
