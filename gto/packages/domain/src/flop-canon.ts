...
// moved here from web/lib/flop-canon.ts (which now re-exports this module's
// twin until web joins the workspace in v1.x).
...
// string is the cache key precomputed solutions are stored under, so any drift
// makes the app look up the wrong (or no) solution. Cross-verified against
// the Python output in flop-canon.test.ts.
//
...
// encoding (cards.ts: card = rank*4 + suit, rank 0=2 … 12=A); these two
// representations are separate and must not be conflated.
const RANKS = "AKQJT98765432";
const SUITS = "cdhs";

function parseCanonCard(card: string): [number, number] {
  return [RANKS.indexOf(card[0]!), SUITS.indexOf(card[1]!)];
}

function canonCardStr(rank: number, suit: number): string {
...
export function canonicalizeBoard(cards: string[]): string {
  const parsed = cards.map(parseCanonCard);
  ...
  for (const perm of SUIT_PERMS) {
    ...
    const candidate = remapped.map(([r, s]) => canonCardStr(r, s));
