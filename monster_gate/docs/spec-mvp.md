...
- 中央 1280×528: **真上から見た正方グリッド**（不思議のダンジョン系と同じ）。1 マス 48×48px。
  カメラはプレイヤーへ毎フレーム 18% ずつ寄る補間追従（7 マス以上のワープ時はスナップ）。
  移動は 130ms で前のマスから補間。
- 下 88px: 手札 10 枠（番号・カード名・MP）＋右端に**こうげきボタン**（Space / A、クリック可）。
  その上にログ 2 行（40px）。
- 描画コードは `src/ui/render/` に分割: `layout`（寸法）/ `grid`（投影・カメラ）/ `art`（素材）/
  `tiles`（コード描画のマス）/ `scene`（マップとキャラ、アニメ状態）/ `hud` / `screens` /
  `card-face`（カード 1 枚）/ `weather`（城ごとの粒）。`app.ts` は状態と入力だけ。
- ミニマップはマップ左上へ半透明で重ねる。
- **描画順は「地面 → 立っている物を Y 昇順」だけ**。真上からは高さが無く何も何かを隠さないので、
  等角時代にあったマスごとの描画キュー・手前の壁による遮蔽・プレイヤーの 50% ゴーストは
  すべて不要になり削除した。同じ理由で床と壁は**明度差でしか区別できない**ため、壁は床より
  はっきり暗く描く。
- キャラ・アイテム・施設は `public/art/manifest.json` に登録されたスプライトで描き、無い ID は
  絵文字のビルボード（影の楕円 + 黒縁 + 足元のリング）にフォールバックする（`docs/style-guide.md` §5）。
  プレイヤーは 3 向き（正面・背面・横）× 3 ポーズ（立ち・踏み出し・振り）を持ち、エンジンの
  8 方向を 4 方向に畳んで引く。横向きは左向きで用意し、右を向いたときはプログラムが反転する。
  素材が 1 枚しか無い ID（モンスターなど）は `<id>` へフォールバックしてそのまま描かれる。
  🦸/🧙/🎲 = 戦士/魔法使い/ギャンブラー、🟢🟡🐝💀🐛👺🔮🐉 = 敵、
