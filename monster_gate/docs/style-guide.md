...
画面の 1 マスは 96×48px（2:1）。以下の「画面上の大きさ」はこの基準。

### キャラ（職業・モンスター）

- 元画像: 透過 PNG、**512×512**、キャラは中央、**接地点を (256, 440)** に揃える。余白は残してよい（manifest の `scale` で合わせる）。
- 画面上: 幅 ≤ 76px、高さ 70〜90px（2.5 頭身）。目安の `scale` は 0.16〜0.18。
- ポーズ: `idle`（必須）→ `attack` / `hurt`（Phase 5）。向きは左。
- ID: `class.warrior` `class.mage` `class.gambler` /
  ...
  ポーズ付きは `enemy.goblin.attack` のように後ろに付ける。
- ポートレート: `portrait.warrior` など。512×512、胸から上（上部バー・拠点・リザルト用）。

### アイテム・施設

- `item.card` `item.win` `item.doubleUp`: 256×256、接地点 (128, 220)。画面上の幅 ≈ 30px。
- `tile.altar` `tile.shop` `tile.casino`: 512×512、接地点 (256, 440)。画面上の幅 ≤ 90px。

### タイル

- 2:1 の菱形。床は 96×(48＋厚み)、壁は 96×(48＋高さ)。接地点は菱形の中心（地面の位置）。
- 元素材（Kenney 等）が別サイズ・別角度なら、`TW/TH` を素材に合わせるか素材側を再サンプルする。**判断は実測後**（Phase 0）。
...
- **城別は `<城 id>.` を頭に付ける**（`cold.tile.floor`）。描画側は「その城の ID → 接頭辞なし」の
  順で引くので、絵の無い城は既定（ゆかいの石）で出る。城の絵は 1 枚ずつ足せる。
- アクセント床 `tile.floor.b` は 12 マスに 1 枚の割合で混ざる。穴の開いた素材を使うときは
  無地の床を下に敷く（穴から背景の黒が抜けて落とし穴に見えるため）。
- `thumb.<城 id>`: 拠点の城選択に出す 3×3 の部屋の隅。**本編と同じタイルで組む**（色見本にしない）。
