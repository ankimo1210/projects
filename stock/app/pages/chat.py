...
def _make_ai_bubble(
    text: str, figures: list[str] | None = None, code_blocks: list[str] | None = None
) -> html.Div:
    children: list = [
            ...
            children.append(
                html.Details(
                    [
                        html.Summary(
                            "実行コード",
                            style={"cursor": "pointer", "color": "#6c757d", "fontSize": "0.85em"},
                        ),
                        html.Pre(
                            code,
                            style={
...
layout = dbc.Container(
    [
        ...
        dbc.Row(dbc.Col(html.H4("AI Chat", className="mb-3"), width=12)),
        # Message area
        dbc.Row(
            dbc.Col(
                html.Div(
...
def poll_result(n_intervals, job_id, history, current_messages):
    ...
    if (
        ...
        and messages[-1].get("props", {}).get("id") == "chat-thinking"
    ):
        messages = messages[:-1]
