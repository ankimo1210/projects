...
| 象限 | 業種 |
|---|---|
| **AI解放** (5) | 不動産業、サービス業、卸売業、情報・通信業、小売業 |
| **人手依存** (11) | 海運業、空運業、倉庫・運輸関連業、建設業、陸運業、ガラス・土石製品、非鉄金属、金属製品、鉱業、石油・石炭製品、水産・農林業 |
| **AI増益** (11) | 機械、保険業、証券、商品先物取引業、精密機器、その他製品、電気機器、化学、医薬品、電気・ガス業、その他金融業、銀行業 |
| **低感応** (6) | 輸送用機器、鉄鋼、パルプ・紙、繊維製品、ゴム製品、食料品 |

建設業は人手不足スコア **100 / AI曝露 28.6**、陸運業は **91.8 / 20.6**。
どちらもこの枠組みでは「AIでは救われない」側にはっきり分離される。
逆に銀行業は **0 / 94.8** で、人手不足ではないがAI余地は最大級 — 成長ではなくコストの話になる。

象限の境界は **33業種の中央値**（人手不足 33.8 / AI 41.6）で一度だけ決め、企業にも同じ境界を
投影して当てる。企業ユニバースの中央値で切り直すと、業種の地図と企業の地図が食い違う。
個別銘柄では 1,642社中 **AI解放 645 / AI増益 607 / 人手依存 218 / 低感応 172**。
...
| 有効求人倍率 | 0.20 | 厚労省「一般職業紹介状況」表21 | 2025年平均 | 1.386倍 |
...
- `ai_potential` は **ILO Working Paper 140**（Gmyrek et al. 2025）の ISCO-08 4桁 427職業の
  ...
  **これはタスク単位の自動化ポテンシャルの職業平均であって、「置き換えられる従業員の割合」
  ではない。** ILO が「Not Exposed」に分類した231職業も 0.09〜0.36 の値を持つ連続量で、
  0/1 の判定ではない。したがって出力列は `ai_exposure_pct`（曝露度）と呼び、
  「AI代替可能な労働の割合」とは呼ばない。
...
この計算は**推定ではなくシナリオ**。曝露が労働時間の解放に等しいという仮定、空いた労働が
同じ生産性で売上に変わるという仮定、その需要が実在するという仮定の上に立ち、導入コストを
引いていない。上限側の目安として読む。

**単体スコープは3値**（`parent_scope`）。「※が付いていない」は「確認できた」ではない。

| 値 | 記号 | 意味 | 1,642社中 |
|---|---|---|---:|
| `confirmed` | — | 単体従業員が連結の20%以上。単体P/Lを事業の代理として読める | 1,035 |
| `unknown` | † | 連結従業員数が取れず判定できない | 78 |
| `thin` | ※ | 20%未満。単体は本社機能に寄り、限界利益率が実態より高く出る | 529 |
...
定めているので、ここで賞与引当を足すと二重計上になる。係数1.18の中身は社会保険料の事業主
負担（現金給与の約15%）と退職給付費用（約3%）だけ。`--benefits-multiplier` で振れる。
旧既定1.25は「平均年間給与は現金給与のみ」という誤った前提に立っており、人件費を約6%
過大に見ていた（人件費は限界利益率の分子に入るので、押上げ余地が上振れしていた）。
...
└── docs/
    ...
    ├── REVIEW-2026-08-17.md         # 第三者レビュー（指摘 F-01〜F-11）
    └── REVIEW-RESPONSE-2026-08-17.md # 指摘への対応と、動いた数字
```
...
- 両軸とも **公表統計を宣言済みのマッピングで33業種に写した値**（`tools/build_reference.py`）。
  analyst の判断が入るのは 6指標の重み・規制ドラッグ・実現率・ISCO と日本標準職業分類の
  対応の4か所。個々のセルの精度ではなく、業種間の順序に意味がある。
- **右上象限は、AIラリー開始以降ユニバースを下回っている**（2022年11月末を100として
  2026年7月末で AI解放 156.3 対 ユニバース 207.9、中央値では +41.6% 対 +71.2%）。
  4象限で最下位で、1位は人手依存（+154.0%）。この枠組みが指す先はまだ株価では報われて
  いない。検証と限界は [docs/REVISION-2026-08-17.md](docs/REVISION-2026-08-17.md)。
  as-of は**最終完了月**を使う（当月の月足は月末値ではない）。
