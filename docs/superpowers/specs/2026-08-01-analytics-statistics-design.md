...
> **CJK 約物と太字の地雷は実在する**（`linear_algebra` が 3 箇所踏んだ）。
> CommonMark は `**` が開くか閉じるかを**両隣の文字**から決め、CJK 約物は punctuation 扱いになる。
> 壊れるのは次の 2 形で、どちらもビルド時に警告が出ない。
>
> - `**「信頼区間」**は難しい` — 閉じ `**` の直前が約物・直後が文字 → 閉じられない。閉じた後に空白を入れる
> - `各章には**「核心」**を置いた` — 開き `**` の直前が文字・直後が約物 → 開けない。括弧を強調の外に出す（`「**核心**」`）
>
> `nbkit.md` が各行をレンダリングして強調が実際に生成されたかを確認し、駄目なら例外を投げる。
> **片側だけを見る正規表現では駄目**（正しい `**実測する**。` を誤検出し、上の 2 形を見逃す）。
...
| プラン | 範囲 | 前提 | 結果 |
|---|---|---|---|
| Plan 1 | M0–M2（足場・確率論のコア・第Ⅰ部 6 章） | なし | **完了 2026-08-01** |
| Plan 2 | M3–M5（推測のコア・第Ⅱ部 5 章・回帰と GLM） | Plan 1 完了 | **完了 2026-08-01** |
| Plan 3 | M6–M7（橋渡し・キャップストーン・演習解答・仕上げ） | Plan 2 完了 | 未着手 |

各プランの完了時に実測値（テスト数・実行時間・図の点数）を記録し、次プランの前提を更新する。

### Plan 1 の実測結果（2026-08-01）

| 項目 | 目標 | 実測 |
|---|---|---|
| テスト | M1 ~20 | **57**（smoke 1・nbkit 4・datasets 8・distributions 8・processes 10・simulation 7・plotting 14・widgets 5） |
| Notebook | 6 章 | 6 章・140 セル |
| インタラクティブ図 | 5 点＋関係図 | **8 点** |
| コールアウト | 核心 6・実社会 6 | 核心 6・実社会 6 |
| NB 実行時間 | 120 秒以内 | **28.3 秒**（第Ⅱ部に 271 秒残る） |
| Notebook 総サイズ | — | 947 KB |

**Plan 2 が引き継ぐ前提**

- 実行時間の残り予算は **271 秒 / 8 章**。07 のブートストラップと 08 の FDR は
  第Ⅰ部の最重章（04 の 4.8 秒）より重くなるが、余裕は十分ある
- `simulation` は設計どおり **拡張せずそのまま使える**。07 の被覆確率も 08 の棄却率も
  既存の `coverage_probability` / `rejection_rate` で書ける
- 図は **ビン集計済みの値だけを埋め込む**こと。`go.Histogram` に生データを渡すと
  1 章が 2.8 MB に膨らんだ（`clt_convergence` で実際に踏んだ）
- アニメーション図は `plotting.core.frame_slider` を通すこと。
  手書きのスライダー配線はテストが弾く
- `widgets` は図を自作してはならない。`.show()` も禁止（ヘッドレスでブロックする）
...
| MyST 約物地雷で本文が崩れる | `nbkit.md` がレンダリングして強調の生成を確認し、駄目なら例外。両側を見る必要がある |

### Plan 2 の実測結果（2026-08-01）

| 項目 | 目標 | 実測 |
|---|---|---|
| テスト | 138 前後 | **142**（estimation 11・intervals 10・testing 13・regression 12・glm 11・plotting_inference 10・plotting_regression 8 ＋ Plan 1 分） |
| Notebook | 11 章 | 11 章・266 セル |
| インタラクティブ図 | 19 点前後 | **19 点** |
| コールアウト | 核心 11・実社会 11 | 核心 11・実社会 11 |
| NB 実行時間 | 190 秒以内 | **52.7 秒**（Plan 3 に 247 秒残る） |
| Notebook 総サイズ | — | 1.58 MB |
| 最も重い章 | — | **NB07 の 13.8 秒**（被覆確率の実験を 4 種類走らせるため） |

**Plan 3 が引き継ぐ前提**

- 実行時間の残り予算は **247 秒 / 3 章**。余裕は十分ある
- `regression` と `glm` は **numpy だけで書いてある**。`statsmodels` は
  テストとノートブックの照合先としてのみ使う。この方針を Plan 3 でも守る
- `tools/build_notebooks.py` は **章番号を引数に取る**。全章を再生成すると
  全章の出力が消えるので、触った章だけを指定すること
- `nbkit.md` は**レンダリングして太字の生成を検査する**。CJK 約物に接する太字は例外になる
- 図は `plotting.core.frame_slider` を通し、生データではなくビン集計値を埋め込む
- **`statsmodels` との標準誤差は 1e-5 までしか一致しない**（係数は 1e-11）。
  原因は収束判定ではなく、`statsmodels` が共分散に最後の反復の作業重み
  （1 つ前の μ から作ったもの）を使うこと。`test_standard_error_gap_is_the_weight_evaluation_point`
  がこの診断を固定している
