...
> ~~MyST admonition 内の太字は CJK 約物で崩れる~~ — 2026-08-01 に実測して撤回。
> markdown-it は `**「これはダメ」**` を含む該当綴りをすべて `<strong>` として描画し、
> `bayesian` / `neural_net` / `linear_algebra` の admonition 56 行が同じ形で正常にビルドされている。
> 元の記憶が指していた問題が何であれ、この機構ではなかった。ガードは置かない。
...
| ~~MyST 約物地雷で本文が崩れる~~ | 2026-08-01 に実測して撤回。想定した機構は再現しなかった |
