# johnhull 06_numerical_methods Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build `hullkit.fd` (theta-scheme finite differences) and Monte Carlo pricing extensions in `hullkit.mc` (European MC + Longstaff-Schwartz), then the 29-cell `volumes/06_numerical_methods/numerical.ipynb` covering Hull 11e Ch.21 and Ch.27.

**Architecture:** As volumes 01–05. Cross-method consistency (CRR vs FD vs MC vs BSM) is the verification axis. Running American-put example params: S=50, K=50, r=10%, σ=40%, T=5/12 (Hull Ch.21 parameters; reviewer verifies any Hull-printed values against the GE PDF).

**Tech Stack:** Python 3.12, numpy/scipy (solve_banded, lstsq), matplotlib + ipympl + ipywidgets, nbformat, pytest.

**Spec:** `docs/superpowers/specs/2026-06-08-johnhull-06-numerical-methods-design.md`

Implementer notes (as previous volumes): ruff lints hullkit src/tests, NOT `build_*_notebook.py`; ruff check + format clean before each commit; commits on main, trailer `Co-Authored-By: Claude Opus 4.8 <noreply@anthropic.com>` after blank line; stage only what each task specifies; statistical tolerances must not be widened silently (report DONE_WITH_CONCERNS with numbers instead).

---

### Task 1: hullkit.fd (TDD)

**Files:**
- Create: `johnhull/hullkit/tests/test_fd.py`
- Create: `johnhull/hullkit/src/hullkit/fd.py`

- [ ] **Step 1: Write the failing tests**

`johnhull/hullkit/tests/test_fd.py`:
...
- [ ] **Step 3: Implement `johnhull/hullkit/src/hullkit/fd.py`**
...
- [ ] **Step 4: Verify** — 6 passed; full suite 57 passed; ruff clean.

- [ ] **Step 5: Commit**

```bash
git add johnhull/hullkit/src/hullkit/fd.py johnhull/hullkit/tests/test_fd.py
git commit -m "feat(hullkit): theta-scheme finite-difference pricer with CRR/BSM cross-checks"
```

---

### Task 2: mc.py Monte Carlo pricing extensions (TDD)

**Files:**
- Create: `johnhull/hullkit/tests/test_mc_pricing.py`
- Modify: `johnhull/hullkit/src/hullkit/mc.py` (append two functions; existing code unchanged)

- [ ] **Step 1: Write the failing tests**

`johnhull/hullkit/tests/test_mc_pricing.py`:
...
def test_antithetic_reduces_se():
    ...
    _, se_anti = mc.price_european_mc(
        **kwargs, antithetic=True, rng=np.random.default_rng(1)
    )
...
def test_mc_put_and_kind_validation():
    ...
    price, se = mc.price_european_mc(
        100.0, 110.0, 0.05, 0.3, 0.5, kind="put", n_paths=200_000
    )
    assert abs(price - target) < 3.0 * se
    with pytest.raises(ValueError):
...
- [ ] **Step 2: Run to verify failure** (AttributeError: no price_european_mc)

- [ ] **Step 3: Append to `johnhull/hullkit/src/hullkit/mc.py`**
...
- [ ] **Step 4: Verify** — 5 passed; full suite 62 passed; ruff clean. Quote LSM value vs CRR reference.

- [ ] **Step 5: Commit**

```bash
git add johnhull/hullkit/src/hullkit/mc.py johnhull/hullkit/tests/test_mc_pricing.py
git commit -m "feat(hullkit): European MC with antithetic variates and Longstaff-Schwartz LSM"
```

---

### Task 3: build script — intro + trees/MC sections (cells 00–11)

**Files:**
- Create: `johnhull/volumes/06_numerical_methods/build_numerical_notebook.py`

- [ ] **Step 1: Create the build script with cells 00–11** (helpers + assembly footer verbatim from `volumes/05_vol_smile_estimation/build_vol_smile_notebook.py`; docstring title `build_numerical_notebook.py`; output `numerical.ipynb`)
...
- [ ] **Step 2: Build and validate** — `Total cells: 12`; nbformat validate → `12 cells valid`.

- [ ] **Step 3: Commit**

```bash
git add johnhull/volumes/06_numerical_methods
git commit -m "feat(johnhull): 06 build script with tree/MC cells"
```

---

### Task 4: build script — FD + Ch.27 + closing (cells 12–28)

**Files:**
- Modify: `johnhull/volumes/06_numerical_methods/build_numerical_notebook.py`

- [ ] **Step 1: Insert cells 12–28 immediately BEFORE the `# ===...` line preceding `# Notebook assembly`**
...
- [ ] **Step 2: Rebuild and validate** — `Total cells: 29`; nbformat validate → `29 cells valid`.

- [ ] **Step 3: Commit**

```bash
git add johnhull/volumes/06_numerical_methods
git commit -m "feat(johnhull): 06 FD/LSM/Merton cells, verification, exercises"
```
...
**Files:**
- Create: `johnhull/volumes/06_numerical_methods/PROGRESS.md`
- Modify: `johnhull/ROADMAP.md` (volume 6 → done; module list gains `fd`)

- [ ] **Step 1: Headless execute** (`--output-dir /tmp --output numerical_executed.ipynb`; cells 23/25 take several seconds each — if a cell exceeds the default timeout, rerun with `--ExecutePreprocessor.timeout=300`). Exit 0 or BLOCKED+traceback.

- [ ] **Step 2: Zero-error + 全チェック合格 heredoc.**

- [ ] **Step 3: Tests + lint:** hullkit suite → 62 passed; ruff johnhull clean; workspace minus gto (explicit paths) → quote summary.

- [ ] **Step 4: Write `johnhull/volumes/06_numerical_methods/PROGRESS.md`**
...
- [ ] **Step 5: ROADMAP volume 6 → done; module list `..., volatility.` → `..., volatility, fd.`**

- [ ] **Step 6: Final commit**

```bash
git add johnhull/volumes/06_numerical_methods/PROGRESS.md johnhull/ROADMAP.md
git commit -m "docs(johnhull): mark 06_numerical_methods complete"
```
