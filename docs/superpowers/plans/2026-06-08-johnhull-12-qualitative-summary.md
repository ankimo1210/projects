# johnhull 12_qualitative_summary Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build the 26-cell `volumes/12_qualitative_summary/qualitative_summary.ipynb` covering Hull 11e Ch.1, 8, 16, 35, 36, 37 (markdown-centric with small compute cells reusing existing hullkit modules). This is the FINAL volume — completing it covers all 37 chapters.

**Architecture:** As volumes 01–11, but NO new hullkit module. Compute cells reuse trees (real option), rates/cost-of-carry (commodity), credit/copula ideas (securitization waterfall, inline), bsm (ESO). Three tasks: build script (2 tasks) + verify/docs/series-completion.

**Spec:** `docs/superpowers/specs/2026-06-08-johnhull-12-qualitative-summary-design.md`

**Reference values** (hand-verified):

| Quantity | Value |
|---|---|
| Commodity F0 (S=100, r=5%, u=2%, y=10%, T=1) | 97.0446 (backwardation, < S0) |
| Abandonment American put (V0=100, K=90, r=5%, σ=30%, T=2, N=500) | 8.3427 |
| Waterfall (pool loss 12%, equity 0-5%, mezz 5-15%, senior 15-100%) | equity 5%, mezz 7%, senior 0% |

Implementer notes: as previous volumes (ruff scope — build script NOT linted; trailer `Co-Authored-By: Claude Opus 4.8 <noreply@anthropic.com>`, stage only specified files, report tolerance failures).

---

### Task 1: build script — intro + Ch.1 + Ch.8 + Ch.16 (cells 00–11)

**Files:**
- Create: `johnhull/volumes/12_qualitative_summary/build_summary_notebook.py`

- [ ] **Step 1: Create the build script with cells 00–11** (helpers + footer verbatim from `volumes/11_ir_derivatives_market/build_ir_options_notebook.py`; title `build_summary_notebook.py`; output `qualitative_summary.ipynb`)
...
```bash
git add johnhull/volumes/12_qualitative_summary
git commit -m "feat(johnhull): 12 build script with intro/securitization/ESO cells"
```

---

### Task 2: build script — Ch.35 + Ch.36 + Ch.37 + capstone (cells 12–25)

**Files:**
- Modify: `johnhull/volumes/12_qualitative_summary/build_summary_notebook.py`

- [ ] **Step 1: Insert cells 12–25 immediately BEFORE the `# ===...` line preceding `# Notebook assembly`**
...
- [ ] **Step 2: Rebuild and validate** — `Total cells: 26`; nbformat validate → `26 cells valid`.

- [ ] **Step 3: Commit**

```bash
git add johnhull/volumes/12_qualitative_summary
git commit -m "feat(johnhull): 12 commodity/real-options/mishaps cells, capstone, verification"
```

---

### Task 3: Headless execution, verification, docs, series completion

**Files:**
- Create: `johnhull/volumes/12_qualitative_summary/PROGRESS.md`
- Modify: `johnhull/ROADMAP.md` (volume 12 → done + series-complete note)

- [ ] **Step 1: Headless execute** (`--output-dir /tmp --output qualitative_summary_executed.ipynb`). Exit 0 or BLOCKED+traceback.

- [ ] **Step 2: Zero-error + 全チェック合格 heredoc.**

- [ ] **Step 3: Tests + lint:** hullkit suite → 96 passed (no new module); ruff johnhull clean; workspace minus gto (explicit paths) → quote summary.

- [ ] **Step 4: Write `johnhull/volumes/12_qualitative_summary/PROGRESS.md`**
...
- [ ] **Step 5: ROADMAP volume 12 → done.** Also add a line at the bottom of `johnhull/ROADMAP.md`:
`**Status (2026-06-08): all 14 rows done → Hull 11e all 37 chapters covered.**`

- [ ] **Step 6: Final commit**

```bash
git add johnhull/volumes/12_qualitative_summary/PROGRESS.md johnhull/ROADMAP.md
git commit -m "docs(johnhull): mark 12_qualitative_summary complete — all 37 chapters covered"
```

- [ ] **Step 7: Report** with quoted outputs and confirm the series is complete.
