# johnhull 05_vol_smile_estimation Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build `hullkit.volatility` (implied vol + EWMA/GARCH estimation, TDD) and the 27-cell `volumes/05_vol_smile_estimation/vol_smile.ipynb` covering Hull 11e Ch.20 and Ch.23.

**Architecture:** As volumes 01–04: hullkit module first (TDD), build script → ipynb, headless nbconvert gate. To reduce GE-section-number churn, markdown cells cite equation numbers (verified) and chapter; § numbers only where certain (Appendix 20A).

**Tech Stack:** Python 3.12, uv workspace, numpy/scipy (brentq, minimize), matplotlib + ipympl + ipywidgets + japanize-matplotlib, nbformat, pytest.

**Spec:** `docs/superpowers/specs/2026-06-08-johnhull-05-vol-smile-estimation-design.md`

**Reference values** (hand-verified; Hull prints in parens):

| Quantity | Exact | Hull prints |
|---|---|---|
| EWMA update (σ_{n−1}=1%/d, u=2%, λ=0.90, Ex 23.1) | var 0.00013, σ 1.1402%/d | 1.14% |
| GARCH update (ω=2e-6, α=0.13, β=0.86; σ_{n−1}=1.6%/d, u=1%) | var 0.00023516, σ 1.5335%/d | 1.53% |
| GARCH long-run | V_L=0.0002, σ_L=1.4142%/d | 1.4% |
| 10-day forecast from σ_n=1.6%/d | 2.50645e-4 | — (eq 23.13) |
| IV round-trip | exact to 1e-8 | — |
| BL flat-σ density | matches lognormal pdf | eq (20A.1)/(20A.2) |

Implementer notes (as previous volumes): ruff lints hullkit src/tests, NOT `build_*_notebook.py`; ruff check + format clean before each commit; commits on main, trailer `Co-Authored-By: Claude Opus 4.8 <noreply@anthropic.com>` after blank line; stage only what each task specifies.

---

### Task 1: hullkit.volatility (TDD)

**Files:**
- Create: `johnhull/hullkit/tests/test_volatility.py`
- Create: `johnhull/hullkit/src/hullkit/volatility.py`

- [ ] **Step 1: Write the failing tests**

`johnhull/hullkit/tests/test_volatility.py`:
...
def test_garch_fit_recovers_persistence():
    ...
    omega_h, alpha_h, beta_h = volatility.garch11_fit(u)
    assert alpha_h + beta_h == pytest.approx(alpha_t + beta_t, abs=0.05)
    assert alpha_h == pytest.approx(alpha_t, abs=0.05)
```

- [ ] **Step 2: Run to verify failure** (`ImportError: cannot import name 'volatility'`)

- [ ] **Step 3: Implement `johnhull/hullkit/src/hullkit/volatility.py`**
...
- [ ] **Step 4: Verify** — 6 passed; full suite 51 passed; ruff check + format clean. The fit test takes a few seconds (Python-loop likelihood) — acceptable; if it exceeds ~60s or fails tolerances, report DONE_WITH_CONCERNS with actual numbers (do not widen silently).

- [ ] **Step 5: Commit**

```bash
git add johnhull/hullkit/src/hullkit/volatility.py johnhull/hullkit/tests/test_volatility.py
git commit -m "feat(hullkit): implied vol and EWMA/GARCH estimation with Hull Ch.20/23 tests"
```

---

### Task 2: build script — intro + Ch.20 (cells 00–12)

**Files:**
- Create: `johnhull/volumes/05_vol_smile_estimation/build_vol_smile_notebook.py`

- [ ] **Step 1: Create the build script with cells 00–12** (helpers + assembly footer copied verbatim from `volumes/04_futures_forwards_rates/build_futures_rates_notebook.py`; docstring title `build_vol_smile_notebook.py`; output `vol_smile.ipynb`)
...
```bash
git add johnhull/volumes/05_vol_smile_estimation
git commit -m "feat(johnhull): 05 build script with vol smile cells"
```

---

### Task 3: build script — Ch.23 + closing (cells 13–26)

**Files:**
- Modify: `johnhull/volumes/05_vol_smile_estimation/build_vol_smile_notebook.py`

- [ ] **Step 1: Insert cells 13–26 immediately BEFORE the `# ===...` line preceding `# Notebook assembly`**
...
- [ ] **Step 2: Rebuild and validate** — `Total cells: 27`; nbformat validate → `27 cells valid`.

- [ ] **Step 3: Commit**

```bash
git add johnhull/volumes/05_vol_smile_estimation
git commit -m "feat(johnhull): 05 GARCH/EWMA cells, verification, exercises"
```
...
**Files:**
- Create: `johnhull/volumes/05_vol_smile_estimation/PROGRESS.md`
- Modify: `johnhull/ROADMAP.md` (volume 5 → done; add `volatility` to the module list)

- [ ] **Step 1: Headless execute** (`--output-dir /tmp --output vol_smile_executed.ipynb`; the GARCH-fit cell takes a few seconds). Exit 0 or BLOCKED+traceback.

- [ ] **Step 2: Zero-error + 全チェック合格 heredoc.**

- [ ] **Step 3: Tests + lint:** hullkit suite → 51 passed; ruff johnhull clean; workspace minus gto (explicit paths) → quote summary.

- [ ] **Step 4: Write `johnhull/volumes/05_vol_smile_estimation/PROGRESS.md`**
...
- [ ] **Step 5: ROADMAP volume 5 → done; module list gains `volatility`.**

- [ ] **Step 6: Final commit**

```bash
git add johnhull/volumes/05_vol_smile_estimation/PROGRESS.md johnhull/ROADMAP.md
git commit -m "docs(johnhull): mark 05_vol_smile_estimation complete"
```
