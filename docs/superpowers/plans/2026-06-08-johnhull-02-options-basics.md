# johnhull 02_options_basics Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build `hullkit.payoffs` (strategy payoff library, TDD) and the 34-cell `volumes/02_options_basics/options_basics.ipynb` covering Hull 11e Ch.10–12, 17, 18, strategy-builder-centric.

**Architecture:** Mirrors volume 01: one new hullkit module + a `build_options_basics_notebook.py` (nbformat-dict pattern) generating the ipynb; premiums priced with the existing `hullkit.bsm` (q parameter covers index/currency/futures), early-exercise charts reuse `hullkit.trees`. Headless nbconvert execution is the integration gate.

**Tech Stack:** Python 3.12, uv workspace, numpy/scipy, matplotlib + ipympl + ipywidgets + japanize-matplotlib, nbformat, pytest.

**Spec:** `docs/superpowers/specs/2026-06-08-johnhull-02-options-basics-design.md`

**Reference values** (hand-verified; Hull-printed approximations in parentheses):
...
| GK currency call | S=K=1.60, r=8%, r_f=11%, σ=14.14%, T=4/12 | 0.0431 | 0.043 — **NOTE: the hull-derivatives skill ref prints "~0.0285", which is WRONG; use 0.0431** |
| Black-76 put | F=K=20, r=9%, σ=25%, T=4/12 | 1.1166 | 1.12 |
| Portfolio-insurance puts | V=$500k, β=2, index=1000, ×100 | 10 | 10 |
| Box spread value | (K2−K1)e^{−rT} | exact identity | — |

Implementer notes (same as volume 01):
- ruff lints hullkit src/tests (line length 100; N803/N806 allowed under johnhull/**) but NOT `build_*_notebook.py` (excluded glob).
- Run `uv run --no-sync ruff check johnhull/hullkit && uv run --no-sync ruff format --check johnhull/hullkit` before every commit.
- All commands from repo root `/home/kazumasa/projects` unless stated. Commits on main; trailer `Co-Authored-By: Claude Opus 4.8 <noreply@anthropic.com>` after a blank line.
- The working tree may contain unrelated uncommitted files from other sessions — stage ONLY what each task specifies.
- Hull figure/example references follow the repo's 11e **Global Edition** PDF (volume-01 lesson).

---

### Task 1: hullkit.payoffs — strategy payoff library (TDD)

**Files:**
- Create: `johnhull/hullkit/tests/test_payoffs.py`
- Create: `johnhull/hullkit/src/hullkit/payoffs.py`

- [ ] **Step 1: Write the failing tests**

`johnhull/hullkit/tests/test_payoffs.py`:
...
Run: `uv run --no-sync pytest johnhull/hullkit/tests/test_payoffs.py -v`
Expected: ERROR — `ImportError: cannot import name 'payoffs'`.

- [ ] **Step 3: Implement `johnhull/hullkit/src/hullkit/payoffs.py`**
...
Run full suite: `uv run --no-sync pytest johnhull/hullkit/tests -q`
Expected: 30 passed.
...
```bash
git add johnhull/hullkit/src/hullkit/payoffs.py johnhull/hullkit/tests/test_payoffs.py
git commit -m "feat(hullkit): strategy payoff library with Hull Ch.12 tests"
```

---

### Task 2: build script — intro + Ch.10 + Ch.11 (cells 00–11)

**Files:**
- Create: `johnhull/volumes/02_options_basics/build_options_basics_notebook.py`

Same nbformat-dict pattern as `volumes/01_foundations/build_foundations_notebook.py`. Filename matches the ruff exclude glob `build_*_notebook.py`.

- [ ] **Step 1: Create the build script with cells 00–11**

`johnhull/volumes/02_options_basics/build_options_basics_notebook.py`:
...
Run: `cd johnhull/volumes/02_options_basics && uv run --no-sync python build_options_basics_notebook.py`
Expected: `Total cells: 12`

Run (same dir): `uv run --no-sync python -c "import nbformat; nb = nbformat.read('options_basics.ipynb', 4); nbformat.validate(nb); print(len(nb.cells), 'cells valid')"`
Expected: `12 cells valid`

- [ ] **Step 3: Commit**

```bash
git add johnhull/volumes/02_options_basics
git commit -m "feat(johnhull): 02_options_basics build script with Ch.10-11 cells"
```

---

### Task 3: build script — Ch.12 strategy cells (cells 12–21)

**Files:**
- Modify: `johnhull/volumes/02_options_basics/build_options_basics_notebook.py`

- [ ] **Step 1: Insert cells 12–21 immediately BEFORE the `# ===...` line preceding `# Notebook assembly`**
...
Run: `cd johnhull/volumes/02_options_basics && uv run --no-sync python build_options_basics_notebook.py`
Expected: `Total cells: 22`

- [ ] **Step 3: Commit**

```bash
git add johnhull/volumes/02_options_basics
git commit -m "feat(johnhull): 02_options_basics Ch.12 strategy cells"
```

---

### Task 4: build script — Ch.17 + Ch.18 + closing (cells 22–33)

**Files:**
- Modify: `johnhull/volumes/02_options_basics/build_options_basics_notebook.py`

- [ ] **Step 1: Insert cells 22–33 immediately BEFORE the `# ===...` line preceding `# Notebook assembly`** (after the Task 3 block)
...
Run: `cd johnhull/volumes/02_options_basics && uv run --no-sync python build_options_basics_notebook.py`
Expected: `Total cells: 34`

Run (same dir): `uv run --no-sync python -c "import nbformat; nb = nbformat.read('options_basics.ipynb', 4); nbformat.validate(nb); print(len(nb.cells), 'cells valid')"`
Expected: `34 cells valid`

- [ ] **Step 3: Commit**

```bash
git add johnhull/volumes/02_options_basics
git commit -m "feat(johnhull): 02_options_basics Ch.17-18 cells, verification, exercises"
```
...
**Files:**
- Create: `johnhull/volumes/02_options_basics/PROGRESS.md`
- Modify: `johnhull/ROADMAP.md` (volume 2 status flip)

- [ ] **Step 1: Execute the notebook headless**

```bash
cd johnhull/volumes/02_options_basics
uv run --no-sync jupyter nbconvert --to notebook --execute options_basics.ipynb --output-dir /tmp --output options_basics_executed.ipynb
```
Expected: exit 0. If it fails, report BLOCKED with the traceback — do not fix content yourself.

- [ ] **Step 2: Assert zero error outputs and verification-cell pass**

```bash
cd /home/kazumasa/projects && uv run --no-sync python - <<'EOF'
import nbformat
nb = nbformat.read("/tmp/options_basics_executed.ipynb", 4)
...
EOF
```
Expected: `error outputs: 0` + confirmation line.

- [ ] **Step 3: Tests + lint (johnhull scope)**

- `uv run --no-sync pytest johnhull/hullkit/tests -q` — expect 30 passed
- `uv run --no-sync ruff check johnhull && uv run --no-sync ruff format --check johnhull/hullkit` — clean
- `uv run --no-sync pytest --ignore=gto/tests -q` — full workspace minus gto; quote the summary line (pre-existing gto collection errors and any failures outside johnhull are documented, not fixed)

- [ ] **Step 4: Write `johnhull/volumes/02_options_basics/PROGRESS.md`**
...
In `johnhull/ROADMAP.md`: `| 2 | `volumes/02_options_basics` | 10, 11, 12, 17, 18 | planned |` → status `done`.

- [ ] **Step 6: Final commit**

```bash
git add johnhull/volumes/02_options_basics/PROGRESS.md johnhull/ROADMAP.md
git commit -m "docs(johnhull): mark 02_options_basics complete in roadmap and progress"
```

- [ ] **Step 7: Report**

Quote actual outputs (nbconvert, step-2 lines, pytest summary). Flag widget interactivity as user-verified-later.
