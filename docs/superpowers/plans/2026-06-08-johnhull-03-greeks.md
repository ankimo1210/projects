# johnhull 03_greeks Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Extend `hullkit.bsm` with analytic Greeks, add `hullkit.hedging` (delta-hedge / stop-loss simulators, TDD), and build the 30-cell `volumes/03_greeks/greeks.ipynb` covering Hull 11e Ch.19.

**Architecture:** Mirrors volumes 01–02: hullkit modules first (TDD), then a build script generating the ipynb, headless nbconvert as the integration gate. The hedging simulators are vectorized over paths (array d1/N(d1) internally) and source paths from `mc.simulate_gbm_paths` (deterministic default seed 42).

**Tech Stack:** Python 3.12, uv workspace, numpy/scipy, matplotlib + ipympl + ipywidgets + japanize-matplotlib, nbformat, pytest.

**Spec:** `docs/superpowers/specs/2026-06-08-johnhull-03-greeks-design.md`

**Reference values** (hand-verified for the Hull §19 running example S=49, K=50, r=5%, σ=20%, T=0.3846; Hull prints in parens):

| Quantity | Exact | Hull prints |
|---|---|---|
| d1 / d2 | 0.05417 / −0.06986 | 0.0542 |
| call price | 2.4004 | 2.40 (×100k shares = $240,000) |
| Δ | 0.5216 | 0.522 |
| Γ | 0.06555 | 0.066 |
| vega (per 1.0 vol) | 12.105 | 12.1 |
| Θ (per year / per cal-day) | −4.3055 / −0.0118 | −4.31 / −0.0118 |
| ρ | 8.9066 | 8.91 |
| BSM PDE | Θ + rSΔ + ½σ²S²Γ = r·c | exact identity |

Implementer notes (same as previous volumes): ruff lints hullkit src/tests but NOT `build_*_notebook.py`; run `uv run --no-sync ruff check johnhull/hullkit && uv run --no-sync ruff format --check johnhull/hullkit` before every commit; commands from repo root; commits on main with trailer `Co-Authored-By: Claude Opus 4.8 <noreply@anthropic.com>` after a blank line; stage ONLY what each task specifies; §/eq/Table citations follow the repo's 11e **GE** PDF.

---

### Task 1: bsm.py Greeks extension (TDD)

**Files:**
- Create: `johnhull/hullkit/tests/test_greeks.py`
- Modify: `johnhull/hullkit/src/hullkit/bsm.py` (append six functions)

- [ ] **Step 1: Write the failing tests**

`johnhull/hullkit/tests/test_greeks.py`:
...
Run: `uv run --no-sync pytest johnhull/hullkit/tests/test_greeks.py -v`
Expected: AttributeError/failures — `bsm` has no `gamma` etc.

- [ ] **Step 3: Append to `johnhull/hullkit/src/hullkit/bsm.py`** (after `put_delta`, keep existing code unchanged)

```python
def gamma(S, K, r, sigma, T, q=0.0):
    """Gamma d2V/dS2 — identical for calls and puts (Hull Ch.19, Table 19.6)."""
    return (
        math.exp(-q * T) * norm.pdf(d1(S, K, r, sigma, T, q)) / (S * sigma * math.sqrt(T))
    )
...
Run: `uv run --no-sync pytest johnhull/hullkit/tests/test_greeks.py -v` → 4 passed.
Full suite: `uv run --no-sync pytest johnhull/hullkit/tests -q` → 34 passed.
Ruff check + format → clean.

- [ ] **Step 5: Commit**

```bash
git add johnhull/hullkit/src/hullkit/bsm.py johnhull/hullkit/tests/test_greeks.py
git commit -m "feat(hullkit): analytic Greeks with Hull Ch.19 example tests"
```

---

### Task 2: hullkit.hedging — delta-hedge / stop-loss simulators (TDD)

**Files:**
- Create: `johnhull/hullkit/tests/test_hedging.py`
- Create: `johnhull/hullkit/src/hullkit/hedging.py`

- [ ] **Step 1: Write the failing tests**

`johnhull/hullkit/tests/test_hedging.py`:
...
def test_mean_is_mu_invariant():
    m_hi = hedging.simulate_delta_hedge(
        **P, n_rebalance=20, n_paths=20_000, mu=0.13,
        rng=np.random.default_rng(1),
    ).mean()
    m_lo = hedging.simulate_delta_hedge(
        **P, n_rebalance=20, n_paths=20_000, mu=0.05,
        rng=np.random.default_rng(1),
    ).mean()
...
def test_performance_improves_with_frequency():
    ...
    for n in (8, 64):
        costs = hedging.simulate_delta_hedge(
            **P, n_rebalance=n, n_paths=10_000, mu=0.13,
...
Run: `uv run --no-sync pytest johnhull/hullkit/tests/test_hedging.py -v`
Expected: ImportError — no `hedging` module.

- [ ] **Step 3: Implement `johnhull/hullkit/src/hullkit/hedging.py`**
...
Run: `uv run --no-sync pytest johnhull/hullkit/tests/test_hedging.py -v` → 5 passed.
If a tolerance fails, report the actual numbers (DONE_WITH_CONCERNS) rather than silently widening it.
Full suite: `uv run --no-sync pytest johnhull/hullkit/tests -q` → 39 passed.
Ruff check + format → clean.

- [ ] **Step 5: Commit**

```bash
git add johnhull/hullkit/src/hullkit/hedging.py johnhull/hullkit/tests/test_hedging.py
git commit -m "feat(hullkit): delta-hedge and stop-loss simulators with convergence tests"
```

---

### Task 3: build script — intro + Greeks tour + hedging centerpiece (cells 00–13)

**Files:**
- Create: `johnhull/volumes/03_greeks/build_greeks_notebook.py`

- [ ] **Step 1: Create the build script with cells 00–13** (same md/code helpers and assembly footer as `volumes/02_options_basics/build_options_basics_notebook.py` — copy those verbatim; output filename `greeks.ipynb`)

The file layout: docstring header (Usage: `uv run python build_greeks_notebook.py`), `md()`/`code()` helpers, `cells = []`, then the cell blocks below, then the standard `# Notebook assembly` section writing `greeks.ipynb`.
...
Run: `cd johnhull/volumes/03_greeks && uv run --no-sync python build_greeks_notebook.py`
Expected: `Total cells: 14`
Validate with nbformat (same one-liner as previous volumes) → `14 cells valid`.

- [ ] **Step 3: Commit**

```bash
git add johnhull/volumes/03_greeks
git commit -m "feat(johnhull): 03_greeks build script with Greeks tour and hedging cells"
```

---

### Task 4: build script — gamma/theta/PDE, joint hedge, rho, insurance, closing (cells 14–29)

**Files:**
- Modify: `johnhull/volumes/03_greeks/build_greeks_notebook.py`

- [ ] **Step 1: Insert cells 14–29 immediately BEFORE the `# ===...` line preceding `# Notebook assembly`**
...
通貨オプションには外国金利 $r_f$ に対する**第2のロー**もあります
...
保有株式比率を $e^{-qT} N(d_1)$ に保ち、残りを無リスク資産へ
（売却比率 $= e^{-qT}[1 - N(d_1)]$）。
...
H_F = e^{−rT_F} H_A = e^{−0.025} × 10,000 ≈ 9,753 株分（eq 19.6）。
...
Run: `cd johnhull/volumes/03_greeks && uv run --no-sync python build_greeks_notebook.py`
Expected: `Total cells: 30`
nbformat validate → `30 cells valid`.

- [ ] **Step 3: Commit**

```bash
git add johnhull/volumes/03_greeks
git commit -m "feat(johnhull): 03_greeks gamma/theta/joint-hedge/insurance cells, verification"
```

---

### Task 5: Headless execution, verification, docs

**Files:**
- Create: `johnhull/volumes/03_greeks/PROGRESS.md`
- Modify: `johnhull/ROADMAP.md` (volume 3 → done)

- [ ] **Step 1: Headless execute** (same command pattern; output `/tmp/greeks_executed.ipynb`). Expect exit 0. BLOCKED + traceback if not.

- [ ] **Step 2: Zero-error + 全チェック合格 assertion** (same heredoc as previous volumes against `/tmp/greeks_executed.ipynb`).

- [ ] **Step 3: Tests + lint:** `uv run --no-sync pytest johnhull/hullkit/tests -q` → 39 passed; ruff check johnhull + format check hullkit → clean; workspace suite minus gto via explicit paths:
`uv run --no-sync pytest market-viz/tests stock/tests nbody-gpu/tests line_backup/tests akinator/tests johnhull/hullkit/tests land_price_api_app -q` → quote summary (expect ~215 passed; failures outside johnhull documented, not fixed).

- [ ] **Step 4: Write `johnhull/volumes/03_greeks/PROGRESS.md`**
...
- [ ] **Step 5: Flip ROADMAP volume 3 to done.**

- [ ] **Step 6: Final commit**

```bash
git add johnhull/volumes/03_greeks/PROGRESS.md johnhull/ROADMAP.md
git commit -m "docs(johnhull): mark 03_greeks complete in roadmap and progress"
```

- [ ] **Step 7: Report** with quoted outputs; flag widget interactivity as user-verified-later.
