...
## Webコードレビュー — 2026-09-05

ボラティリティーViewerの既存操作をレビューし、次の3点を修正。

- CSVの非同期読込完了が、後から選んだプリセットを上書きしていた。
  読込番号で現在の操作だけを採用し、古い成功・失敗・完了処理を破棄する。
  プリセットに戻った後、新しいCSVの読込中表示も古い読込では解除しない。
- 満期・K/Fのスライダーが、読み上げには配列番号しか渡していなかった。
  表示値を実際のスライダーの `aria-valuetext` に渡し、CSV読込や選択変更にも追随させる。
- Babylonのホイール拡大でページもスクロールしていた（再現時は35→235px）。
  カメラ入力でブラウザの既定動作を抑止し、3種類ともページ位置を保って拡大する。

追加したPlaywright 5件は修正前にすべて意図した理由で失敗し、修正後に成功。
`npm test` はNode 7件・Playwright 21件成功（1.9分）。型チェック・lint・本番ビルド成功。
Pythonの既存5件と4ファイルの構文チェックも成功。変更差分の独立レビューも実施。
ブラウザ検証はLinux Chromium / SwiftShader。実機の読み上げソフトとiPhone/Safariは未検証。
大きなJSチャンクのビルド警告は引き続き残る。

## ボラティリティーサーフェスViewer — 2026-09-04
