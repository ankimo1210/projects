...
def candlestick_chart(
    ...
    if ma_windows:
        ...
        for i, w in enumerate(ma_windows):
            ...
            fig.add_trace(
                go.Scatter(x=df["timestamp"], y=ma, name=f"MA{w}",
    ...
    fig.update_layout(
        template=DARK_TEMPLATE, xaxis_rangeslider_visible=False,
...
def line_chart(
    ...
    fig.update_layout(
        template=DARK_TEMPLATE, title_text=title, height=height,
...
def correlation_heatmap(corr_matrix: pd.DataFrame, title: str = "Correlation Heatmap") -> go.Figure:
    ...
    fig.update_layout(
        template=DARK_TEMPLATE, title_text=title,
...
def equity_chart(equity: pd.Series, title: str = "Equity Curve") -> go.Figure:
    ...
    fig.update_layout(
        template=DARK_TEMPLATE, title_text=title, height=350,
        margin=dict(l=10, r=10, t=30, b=10),
    )
    return fig
...
def drawdown_chart(equity: pd.Series) -> go.Figure:
    ...
    fig = go.Figure(go.Scatter(
        ...
        fill="tozeroy", line_color="#ef5350",
    ))
    fig.update_layout(
        template=DARK_TEMPLATE, title_text="Drawdown (%)", height=250,
        margin=dict(l=10, r=10, t=30, b=10),
    )
    return fig
