...
from src.analytics.returns import build_return_matrix
from src.analytics.volatility import build_vol_matrix
from src.analytics.zscore import build_zscore_matrix
from src.analytics.drawdown import build_drawdown_matrix
        ...
        if len(rv) >= 2 and pd.notna(rv.iloc[-1]) and pd.notna(rv_60.iloc[-1]):
            if rv_60.iloc[-1] > 0 and rv.iloc[-1] / rv_60.iloc[-1] >= vol_spike_mult:
                alerts.append({
                    ...
                    "triggered_at": now,
                    "message": (
                        f"{ticker} vol spike: 20d={rv.iloc[-1]:.1%} vs "
                        f"60d={rv_60.iloc[-1]:.1%}"
                    ),
                })
