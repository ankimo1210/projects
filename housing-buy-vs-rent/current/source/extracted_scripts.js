...
const HousingModel = (() => {
 ...
 function saleTax(p,price=null,years=p.years){
  ...
  // Transaction costs carry a price-proportional rate and a flat yen amount, so a
  // break-even search reprices the brokerage sliding scale at every candidate price.
  const basisInitial=p.price*(1+p.basis_cost)+p.basis_cost_fixed,B=basisInitial*p.building_share;
  const proceeds=P*(1-p.sell_cost)-p.sell_cost_fixed;
  ...
  const taxable=Math.max(0,proceeds-basis-p.deduction_sale);
  let t=0;
  if(p.sale_tax){
  ...
  return {tax:t,taxable_gain:taxable,depreciation:dep,basis,price:P,net_before_debt:proceeds-t};
 }
 function run(p,allMonths=false){
  ...
  const initBuy=p.price*(1-p.ltv+p.buy_cost)+p.buy_cost_fixed,initRent=p.rent*p.rent_initial,K=Math.max(initBuy,initRent);
...
// ===== script block 2 =====
const BASE_PARAMS={"price": 250000000, "rent": 750000, "years": 10, "salary": 50000000, "social": 1500000, "social_corp": 1500000, "other_deduction": 0, "sacrifice": 1.0, "cap": 0, "fringe": 0, "corp_years": 10, "ltv": 0.9, "mortgage_rate": 0.015, "mortgage_years": 35, "loan_type": "amortizing", "buy_cost": 0.07, "buy_cost_fixed": 0, "sell_cost": 0.035, "sell_cost_fixed": 0, "owner_cost": 2500000, "rent_initial": 2.0, "renewal": 1.0, "growth": 0, "rent_growth": 0, "owner_growth": 0, "invest": 0.03, "building_share": 0.3, "basis_cost": 0.04, "basis_cost_fixed": 0, "deduction_sale": 30000000, "sale_tax": true, "reduced_long": true, "extra_repair": 0, "repair_year": 8, "step_year": 0, "step_rate": 0.03};
...
const HousingSensitivity = (() => {
 ...
 const specs=[
  ...
  ['buy_cost_fixed','static','取引','購入諸費用の定額分','c_B0','万円',1e4,0,1000,1,'価格に比例しない部分。仲介手数料の速算式の定額、登記・印紙など。','仮定'],
  ['sell_cost','static','取引','売却費用率','c_S','%',.01,0,10,.1,'損益分岐として求める売値にも比例。','仮定'],
  ['sell_cost_fixed','static','取引','売却費用の定額分','c_S0','万円',1e4,0,1000,1,'売値に比例しない部分。分岐売値を探索するときも同額を差し引く。','仮定'],
  ...
  ['basis_cost_fixed','static','売却税','取得費に入れる定額分','c_A0','万円',1e4,0,1000,1,'購入諸費用の定額分のうち取得費に算入できる額。定額分以下。','要資料確認'],
 ...
 function validate(p){
  ...
  if(p.basis_cost_fixed>p.buy_cost_fixed)return '税務取得費に算入する定額分が購入諸費用の定額分を超える';
...
const HousingInputs=(() => {
 ...
 const modeOptions={
  ...
  rent:['manual','yield_linked'],
  deduction_sale:['manual','residential','non_residential'],
  sell_cost:['manual','itemized']
 };
 // 不動産譲渡契約書の印紙税（軽減後、令和9年3月31日まで）。500万円以下は未確認のため最小区分と同額。
 const STAMP_DUTY=[[10000000,5000],[50000000,10000],[100000000,30000],[500000000,60000],[1000000000,160000],[5000000000,320000],[Infinity,480000]];
 const stampDuty=amount=>STAMP_DUTY.find(([limit])=>amount<=limit)[1];
 // 仲介手数料の速算式（税込）と、マンションの抵当権抹消（土地1＋建物1）。
 const BROKERAGE_RATE=.033,BROKERAGE_FIXED=66000,MORTGAGE_RELEASE=2000;
 const sellCostKeys=['sell_cost','sell_cost_fixed'];
 // The relief preset drives reduced_long too, so editing either one returns both to manual.
 const reliefKeys=['deduction_sale','reduced_long'];
 // Which parameters each link writes. A link driving two of them has to hand both back
 // when it is switched off, or the one the user did not type would snap to a stale value.
 const drivenKeys={deduction_sale:reliefKeys,sell_cost:sellCostKeys};
 ...
 // Added after schema 1; an older saved file omits them and keeps the zero default.
 const optionalKeys=new Set(['buy_cost_fixed','sell_cost_fixed','basis_cost_fixed']);
 const optionalModes=new Set(['rent','deduction_sale','sell_cost']);
 function create(parameters=BASE_PARAMS){
  const values={};
  for(const key of keys){
   const supplied=parameters?.[key];
   const value=supplied===undefined&&optionalKeys.has(key)?BASE_PARAMS[key]:supplied;
   if(typeof value!==typeof BASE_PARAMS[key]||(typeof value==='number'&&!Number.isFinite(value)))throw new Error(key+'の値または型が不正です。');
   values[key]=value;
  }
  // Captured from the manual pair so that switching the link on leaves the rent where it is.
  return {values,modes:manualModes(),ageBand:'under40',rentYield:values.rent*12/values.price};
 }
 ...
 // Copy what the link currently produces into the stored values, so dropping it keeps
 // the numbers the user is looking at instead of reverting to whatever was stored before.
 function materialize(state,keys){
  const next={...state,values:{...state.values},modes:{...state.modes}};
  let resolved;
  try{resolved=resolve(state).parameters;}catch{return next;}
  for(const key of keys)for(const driven of drivenKeys[key]||[key])next.values[driven]=resolved[driven];
  return next;
 }
 function setMode(state,key,mode){
  if(!modeOptions[key]?.includes(mode))throw new Error('連動モードが不正です。');
  const base=mode==='manual'&&state.modes[key]!=='manual'?materialize(state,[key]):state;
  return {...base,values:{...base.values},modes:{...base.modes,[key]:mode}};
 }
 ...
 function update(state,patch){
  ...
  for(const key of Object.keys(patch)){
   if(modeOptions[key]&&state.modes[key]!=='manual')dropping.add(key);
   for(const [mode,keys] of Object.entries(drivenKeys))if(keys.includes(key)&&state.modes[mode]!=='manual')dropping.add(mode);
  }
  let next=dropping.size?materialize(state,[...dropping]):{...state,values:{...state.values},modes:{...state.modes}};
  next={...next,values:{...next.values},modes:{...next.modes}};
  for(const [key,value] of Object.entries(patch)){
   ...
   if(reliefKeys.includes(key))next.modes.deduction_sale='manual';
   if(sellCostKeys.includes(key))next.modes.sell_cost='manual';
  }
  if(next.modes.rent==='manual')next.rentYield=next.values.rent*12/next.values.price;
  return next;
 }
 function setRentYield(state,rentYield){
  if(!Number.isFinite(rentYield)||rentYield<=0)throw new Error('表面利回りが不正です。');
  return {...state,values:{...state.values},modes:{...state.modes},rentYield};
 }
 // Rates verified 2026-09-18 against the primary sources cited in the report.
 // childSupport is the separate 子ども・子育て支援金 scheme, not part of the health rate;
 // it is also split with the employer and shares the same standard monthly cap, so it is
 // added before halving. Keeping it apart matters because the two rates change separately.
 const SOCIAL_RATES={health:.0985,childSupport:.0023,care:.0162,pension:.183,employment:.005};
 const SOCIAL_CAPS={health:1390000,pension:650000};
 function estimateSocial(gross,ageBand){
  const monthly=Math.max(0,gross/12);
  const care=ageBand==='40-64'?SOCIAL_RATES.care:0;
  const health=12*Math.min(monthly,SOCIAL_CAPS.health)*(SOCIAL_RATES.health+SOCIAL_RATES.childSupport+care)/2;
  const pension=12*Math.min(monthly,SOCIAL_CAPS.pension)*SOCIAL_RATES.pension/2;
  return health+pension+Math.max(0,gross)*SOCIAL_RATES.employment;
 }
 function resolve(state){
  ...
  if(!Number.isFinite(state.rentYield)||state.rentYield<=0)throw new Error('表面利回りが不正です。');
  ...
  // Rent resolves first: the corporate-housing premium estimate reads it.
  if(state.modes.rent==='yield_linked')assign('rent',parameters.price*state.rentYield/12,'yield_linked',['price','rentYield']);
  ...
  // The stamp bracket is pinned to the forecast sale price, but the flat total is
  // subtracted at every candidate price, so a break-even search keeps the sliding scale.
  if(state.modes.sell_cost==='itemized'){
   const expected=parameters.price*Math.pow(1+parameters.growth,parameters.years);
   assign('sell_cost',BROKERAGE_RATE,'itemized',['price','growth','years']);
   assign('sell_cost_fixed',BROKERAGE_FIXED+stampDuty(expected)+MORTGAGE_RELEASE,'itemized',['price','growth','years']);
  }
  // A preset for the居住用財産 assumptions; it does not check eligibility.
  if(state.modes.deduction_sale!=='manual'){
   const residential=state.modes.deduction_sale==='residential';
   assign('deduction_sale',residential?30000000:0,state.modes.deduction_sale,[]);
   assign('reduced_long',residential,state.modes.deduction_sale,[]);
  }
 ...
 function breakAtEntryPrice(state,keepExit){
  ...
  // A rent tied to the price must stay inside its own range, or resolve() rejects the candidate.
  if(state.modes.rent==='yield_linked'){
   const rentSpec=S.byKey.rent;
   lo=Math.max(lo,rentSpec.min*rentSpec.scale*12/state.rentYield);
   hi=Math.min(hi,rentSpec.max*rentSpec.scale*12/state.rentYield);
  }
  if(keepExit){
 ...
 function serialize(state){
  ...
  return JSON.stringify({model:'housing-input-state',schemaVersion:2,taxRuleSet:'jp-2026-fixed',units:'JPY and decimal rates',inputs:state,parameters},null,2);
 }
 function deserialize(document){
  ...
  if(document.model!=='housing-input-state'||![1,2].includes(document.schemaVersion)||document.taxRuleSet!=='jp-2026-fixed')throw new Error('設定JSONの版が未対応です。');
  const supplied=document.inputs;
  if(!supplied||keys.some(key=>!optionalKeys.has(key)&&supplied.values?.[key]===undefined))throw new Error('入力設定が不足しています。');
  const state=create(supplied.values);
  // Only links introduced after schema 1 may be absent; the original five stay required.
  for(const key of Object.keys(modeOptions))state.modes[key]=supplied.modes?.[key]??(optionalModes.has(key)?'manual':undefined);
  state.ageBand=supplied.ageBand;
  if(supplied.rentYield!==undefined)state.rentYield=supplied.rentYield;
  const {parameters}=resolve(state);
  if(!document.parameters)throw new Error('保存済みの計算値がありません。');
  // A schema-1 file predates the flat cost amounts, so only compare what it stored.
  if(keys.some(key=>!optionalKeys.has(key)&&document.parameters[key]===undefined))throw new Error('保存済みの計算値が不足しています。');
  if(keys.some(key=>document.parameters[key]!==undefined&&document.parameters[key]!==parameters[key]))throw new Error('保存済みの計算値が入力と一致しません。');
  return state;
 }
 return {create,createDefault,setMode,setAgeBand,setRentYield,update,estimateSocial,resolve,scenario,evaluate,breakPrice,breakEntry,serialize,deserialize};
})();
...
if(typeof document!=='undefined')(()=>{
 ...
 function recalc(){
  try{
   ...
   $('sim-assumptions').textContent='社会保険料（年額）通常 '+fmtMan(p.social)+'（'+socialSource('social')+'） / 社宅 '+fmtMan(p.social_corp)+'（'+socialSource('social_corp')+'）、保有費用 '+fmtMan(p.owner_cost)+' / 年（'+(state.modes.owner_cost==='estimate'?'価格の1%で概算':'手入力')+'）。価格予想 '+fmtPct(p.growth)+' / 年、売却特別控除 '+fmtMan(p.deduction_sale)+'（'+(state.modes.deduction_sale==='manual'?'手入力':state.modes.deduction_sale==='residential'?'居住用の特例を適用する仮定':'特例なし')+'）。'+(state.modes.rent==='yield_linked'?'家賃は表面利回り '+(state.rentYield*100).toFixed(2)+'% で買値に連動。':'家賃は入力額で固定。');
 ...
 function renderModes(){
  for(const key of ['social','social_corp','owner_cost','owner_growth','rent','deduction_sale','sell_cost']){
   $('mode-'+key).value=state.modes[key];
   $('f-'+key).disabled=state.modes[key]!=='manual';
  }
  // The relief preset owns the reduced-rate flag as well.
  $('f-reduced_long').disabled=state.modes.deduction_sale!=='manual';
 ...
 function setupInputControls(){
  ...
  const modes={social:[['estimate','年収から概算'],['manual','年額を入力']],social_corp:[['estimate','社宅時の給与から概算'],['same_as_normal','通常時と同額'],['manual','年額を入力']],owner_cost:[['estimate','購入価格の1%で概算'],['manual','年額を入力']],owner_growth:[['manual','増加率を入力'],['same_as_rent','家賃増加率と同じ']],rent:[['manual','月額を入力'],['yield_linked','買値と同じ表面利回りで連動']],deduction_sale:[['manual','控除額を入力'],['residential','居住用（3,000万円控除と10年超軽減）'],['non_residential','居住用以外（特例なし）']],sell_cost:[['manual','率を入力'],['itemized','仲介手数料・印紙税・抹消から計算']]};
