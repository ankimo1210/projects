...
print(f"Vega (per 1bp):")
print(f"  Analytical: {vega_analytical:.8f}")
print(f"  Finite Diff: {vega_fd:.8f}")

# Check relative error, but adjust for small numbers
rel_error = abs(vega_analytical - vega_fd) / max(vega_fd, vega_analytical) * 100 if max(vega_fd, vega_analytical) > 0 else 0
...
for T, F, a_true, r_true, n_true in zip(EXPIRIES_T8, ATM_FWDS_T8, TRUE_ALPHA, TRUE_RHO, TRUE_NU):
