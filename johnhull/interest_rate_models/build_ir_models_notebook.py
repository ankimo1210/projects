...
cells.append(
    md(r"""> **核心** — 短期金利の動学を1本の SDE で記述し、カーブ全体を生成する——それが金利モデル。<br>
> **直感** — 平均回帰・正規/対数正規・1因子/多因子の選択が、負金利の可否やカーブ形状を決める。<br>
> **実務** — 債券・スワプション・ALM の値付けとリスク管理。モデル選択がそのまま商品性能に出る。""")
)
...
cells.append(
    md(r"""> **核心** — 平均回帰する正規モデル。閉形式の債券価格を持つ。<br>
> **直感** — 金利は長期水準 θ へ κ の速さで戻る。正規なので負金利を許す。<br>
> **実務** — 解析解があり教育・基準に最適。負金利を許す点が低金利環境ではむしろ現実的。""")
)
...
cells.append(
    md(r"""> **核心** — 拡散項に √r を付け、金利を非負に保つ平均回帰モデル。<br>
> **直感** — 金利が低いほどボラも小さくなり 0 で反射。Feller 条件で正値を保証。<br>
> **実務** — 金利・信用強度・確率分散(Heston)の基礎。非負性が要る量のモデル化に。""")
)
...
cells.append(
    md(r"""> **核心** — Vasicek に時間依存ドリフトを足し、今日のカーブに完全フィットさせる。<br>
> **直感** — θ(t) を調整して初期イールドカーブを厳密に再現。アービトラージフリー。<br>
> **実務** — 金利デリバティブの業界標準的モデル。市場カーブ整合が実務の必須要件。

> **実務での出番 — なぜ『今日のカーブにフィット』が必須か**
>
> Vasicek/CIR は少数パラメータゆえ、観測される今日のイールドカーブを正確には再現できない。だが債券オプションを値付けるには、まず原資産(債券)の現在価格＝カーブを正しく出す必要がある。Hull–White は時間依存ドリフト θ(t) でカーブを厳密に再現する——だから金利デリバの実務標準になった。『モデルは市場価格を再現してから初めて使える』という鉄則の典型。""")
)
...
cells.append(
    md(r"""> **核心** — フォワードレート曲線全体の動学を直接記述する枠組み。<br>
> **直感** — 無裁定がドリフトをボラ構造から一意に決める(HJM ドリフト条件)。<br>
> **実務** — 短期金利モデルを包含する一般理論。実装はマルコフ性回復(Hull–White 等)が鍵。""")
)
...
cells.append(
    md(r"""> **核心** — 観測可能な LIBOR フォワードを対数正規でモデル化(LIBOR Market Model)。<br>
> **直感** — 各フォワードが Black 型 → キャップ/スワプションの市場慣行と直結。<br>
> **実務** — 金利エキゾチックの主力。市場の Black 公式と整合する点が実務的強み。""")
)
