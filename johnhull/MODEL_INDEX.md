...
| CEV, Merton jump diffusion, variance gamma | Hull 11e GE §27.1 | `hullkit.alternative_models:cev_price`, `hullkit.alternative_models:merton_jump_price`, `hullkit.alternative_models:variance_gamma_price` | `test_alternative_models.py`, `test_alternative_models_lesson.py` | vol 06 | β=1, λ=0, ν=0 limits; CEV independent PDE (3 strikes); Merton original-Poisson expectation (26 strikes); VG gamma-density integral (26 strikes); Table 27.1; Book/portal shared figures |
| §27.1 saved-data lesson (private) | Hull 11e GE §27.1 | `hullkit._alternative_models_lesson:_figures` | `test_alternative_models_lesson.py` | vol 06 | Reference/source hash guard and identical four Plotly data/layout payloads for Book and portal |
| Time-dependent and stochastic volatility (average variance, Hull–White mixing, Heston by COS) | Hull 11e GE §27.2 eqs. 27.1–27.3; Hull & White (1987); Heston (1993) | `hullkit.stochastic_volatility:average_variance_rate`, `hullkit.stochastic_volatility:time_dependent_bsm_price`, `hullkit.stochastic_volatility:expected_average_variance`, `hullkit.stochastic_volatility:simulate_average_variance`, `hullkit.stochastic_volatility:mixing_price`, `hullkit.stochastic_volatility:heston_price` | `test_stochastic_volatility.py`, `test_stochastic_volatility_reference.py` | vol 06 | Hull 0.065 / 25.5% pinned; independent time-dependent-vol CN PDE (3 strikes, max 1.6e-4); Hull–White conditional MC vs independent Gil-Pelaez Heston (26 strikes, ≤0.31 SE); COS vs Gil-Pelaez (78 prices, 1.6e-13); ρ=0 smile symmetric to 1.8e-15; BSM(√E[V̄]) overprices K=88–128; SABR via `hullkit.sabr:sabr_implied_vol` equals Hull's printed formula (3.1e-16) and an Euler MC within 1.5 SE; `docs/validation/section-27-2/` |
| §27.2 saved-data lesson (private) | Hull 11e GE §27.2 | `hullkit._stochastic_volatility_lesson:_figures` | `test_stochastic_volatility_lesson.py` | vol 06 | Reference/source hash guard and identical four Plotly data/layout payloads for Book and portal |
...
| Barrier static call ladder | Hull §26.17, Table 26.1 | `hullkit.static_replication:StaticCallHedge`, `hullkit.static_replication:up_and_out_call_hedge` | `test_static_replication.py`, `test_static_replication_reference.py` | vol 10 §4.7 | Triangular boundary matching at 3/18/100 nodes; initial values 0.7303/0.3776/0.3249 versus independently integrated continuously monitored barrier value 0.313571; `docs/validation/section-26-17/` |
...
| `hullkit._variance_swap_lesson` | Hull §26.16 のボラ/バリアンス給付と名目換算・Example 26.4 のストリップ・式26.8の格子/翼の誤差・式26.9と厳密 E(√V)/MC を保存済みデータから共有する内部 Plotly 図ビルダー（`test_variance_swap_lesson.py`） |
| `hullkit._static_replication_lesson` | Hull §26.17 の境界・Table 26.1 の脚・節点間残差・3/18/100 点収束を保存済みデータから共有する内部 Plotly 図ビルダー（`test_static_replication_lesson.py`） |
