...
数値・図・本文／配布のTask1–3、最終ブランチレビュー、修正後の限定再レビューはすべて承認済み。未解決指摘なし。判定は統合記録を参照。

- 独立42価格を固定し、別方式のCRRでN128/256/512/1024の収束を実測。採用N1024の最大絶対残差0.003792916424、許容差0.005。経験的検査であり全域の誤差上限ではない。
- 本文6小節・共有4図、vol10は74セル。保存／別実行の出力比較、4図data/layout一致、全セルの種類・ID・本文と現builderの一致を確認。既存ソースの変更はAsian見出し4.2→4.3のみ。
- portalは12テーマ94図（exotics14図）。Book31ページbuild成功、44警告（Plotly MIME38、既存見出し5、報告総数との差1）。HTML fallbackは実ブラウザで確認。
- Chromium141でBook/portal×13状態×1440/1000幅＝52状態の数値と配置を確認。18画像を保存し、負の現金脚を含む誤配分も合計値を変えずに検出・復元した。Book数式42要素・エラー0、両面pageerror0、portal外部要求0。BookのMathJax CDN依存は残る。
- 新旧4節の回帰は **622 passed / 6 skipped（16.73秒）**。§26.9・§26.10・§26.11も最終Book/portalで実画面を再検査し、新しいm4b-recheckを現行台帳へ接続した。過去の18数値・統合JSON、lookbackの日時付きブラウザ記録と18画像を保持した。
- 全体pytestは **1,916 passed / 6 skipped / 2 warnings（86.05秒）**。警告2件は既存japanize_matplotlib/distutilsの非推奨警告。6スキップはM4aのlookback上界検査におけるr=qの6行であり、新shout実装はr=qも検査済み。
- lookback上界の実測範囲は36行・6市場、二参照エンジンの価格相互比較はATM14行。putは原典外の独立拡張。境界の実ノード幅は連続境界の包含保証ではなく、価格精度と別に示す。
- 台帳は受入4・不足0・未評価302。台帳ガードは全体テストに含まれ、成果物ハッシュ付きCLI検査も成功。公開価格API・本番依存は追加していない。
