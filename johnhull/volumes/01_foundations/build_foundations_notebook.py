...
cells.append(
    code(r"""# 2ステップ ヨーロピアンコール（S0=20, K=21, u=1.1, d=0.9, Δt=0.25）
# 11e Global Edition の Fig 13.4 は r=4%（0.9497）、US 版の古典例は r=12%（1.2823）
_, opt_ge = trees.binomial_tree(20.0, 21.0, 0.04, 0.5, 2, u=1.1, d=0.9)
_, opt = trees.binomial_tree(20.0, 21.0, 0.12, 0.5, 2, u=1.1, d=0.9)
print(f"2ステップ コール r=4%  = {opt_ge[0][0]:.4f}（Hull GE Fig 13.4: 0.9497）")
print(f"2ステップ コール r=12% = {opt[0][0]:.4f}（US 版: 1.2823）")
...
# Hull §13.1 1ステップ: S0=20, u=1.1, d=0.9, K=21, T=0.25
# 11e Global Edition（johnhull/ の PDF）は r=4% で f=0.545
stock1_ge, opt1_ge = trees.binomial_tree(20.0, 21.0, 0.04, 0.25, 1, u=1.1, d=0.9)
checks.append(("1ステップ コール 0.545（GE, r=4%）", float(opt1_ge[0][0]), 0.545, 5e-4))
checks.append(("1ステップ Δ = 0.25", trees.tree_delta(stock1_ge, opt1_ge), 0.25, 1e-9))
# US 版の古典例 r=12% は f=0.633
stock1, opt1 = trees.binomial_tree(20.0, 21.0, 0.12, 0.25, 1, u=1.1, d=0.9)
checks.append(("1ステップ コール 0.633（US, r=12%）", float(opt1[0][0]), 0.633, 5e-4))

# Hull Fig 13.4 2ステップ コール（GE r=4%: 0.9497、US r=12%: 1.2823）
_, opt2_ge = trees.binomial_tree(20.0, 21.0, 0.04, 0.5, 2, u=1.1, d=0.9)
checks.append(("2ステップ コール 0.9497（GE Fig 13.4）", float(opt2_ge[0][0]), 0.9497, 5e-5))
_, opt2 = trees.binomial_tree(20.0, 21.0, 0.12, 0.5, 2, u=1.1, d=0.9)
checks.append(("2ステップ コール 1.2823（US）", float(opt2[0][0]), 1.2823, 5e-4))
