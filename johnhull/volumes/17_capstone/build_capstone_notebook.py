...
cells.append(
    md(r"""> **核心** — 1つのポジションに、全巻の道具(確率解析・確率ボラ・数値・XVA)を一気通貫で適用。<br>
> **直感** — 価格 → リスク中立性 → 数値効率 → ヘッジ → 相手方信用、と背骨を辿る。<br>
> **実務** — 実デスクが新規取引で踏む工程の縮図。道具がどこで効くかを体感する。""")
)
...
cells.append(
    md(r"""> **核心** — Heston で値付け(A2) → 測度の正体(A1) → 数値で高速化(A3)。<br>
> **直感** — 同じ価格を、モデル・測度・数値の3視点で確かめる。<br>
> **実務** — 値付けの妥当性は『複数の道で同じ値』で担保する。""")
)
...
cells.append(
    md(r"""> **核心** — デルタヘッジ後も、ベガ・ガンマ・モデルリスクが残る。<br>
> **直感** — 1次を消しても2次・モデル誤差は残る。完全ヘッジは幻想。<br>
> **実務** — 残存リスクの把握と限度管理がデスクの実務。""")
)
...
価格は「リスクフリー価格 − CVA」と調整される——危機後の XVA 実務の基本。""")
)
cells.append(
    md(r"""> **核心** — ヘッジ取引自体に相手方信用リスク(CVA)が乗る。<br>
> **直感** — リスクを移すと別のリスク(信用)が生まれる。<br>
> **実務** — ヘッジのコストは CVA 込みで評価する。XVA デスクとの連携。""")
)
...
cells.append(
    md(r"""> **核心** — 確率ボラを消すと Heston → BSM。高度モデルは古典の一般化。<br>
> **直感** — 極限で既知の答えに戻る、がモデル健全性の検算。<br>
> **実務** — 新モデルは『既知の特殊形に帰着するか』で必ず検証する。

> **実務での出番 — 極限チェックはモデルの『健康診断』**
>
> 新しい価格モデルを実装したら、必ずパラメータを極限に飛ばして既知の答えに戻るか確かめる：Heston は vol-of-vol→0 で BSM、SABR は ν→0 で対数正規、二項木は N→∞ で BSM。この『collapse テスト』はバグ検出の最強の手段であり、複雑なモデルが古典の一般化であることの確認でもある。""")
)
