...
cells.append(
    md(r"""> **核心** — Greeks は『価格が各リスク要因にどれだけ反応するか』の地図。<br>
> **直感** — 価格 V を S, σ, t, r で偏微分したもの。ポジションを要因ごとに分解して管理する。<br>
> **実務** — トレーディング・デスクの損益と在庫はすべて Greeks で集計・限度管理される。日次の必須言語。""")
)
...
cells.append(
    md(r"""> **核心** — Δ=∂V/∂S, Γ=∂²V/∂S², ν=∂V/∂σ, Θ=∂V/∂t, ρ=∂V/∂r。<br>
> **直感** — 1次感応度（Δ, ν, ρ）と2次・時間（Γ, Θ）。Γ と Θ は表裏の関係。<br>
> **実務** — リスクレポートは各 Greek を通貨額に換算して合算し、限度に当てて建玉を調整する。""")
)
...
cells.append(
    md(r"""> **核心** — 1つのコール売りポジションを題材に、全 Greek を一気通貫で見る。<br>
> **直感** — ネイキッド（無ヘッジ）は方向リスク満載。まず Δ を消し、次に Γ・ν を整える。<br>
> **実務** — 新規建玉を取った瞬間に『何に晒されるか』を把握する——デスクの初動そのもの。""")
)
...
cells.append(
    md(r"""> **核心** — 配当利回り q を入れれば、株・指数・通貨・先物のデルタを1式で扱える。<br>
> **直感** — 先物でヘッジするなら必要枚数は H_F=e^{−(r−q)T}H_A。キャリーの分だけ調整。<br>
> **実務** — 現物でなく先物でデルタヘッジするのが実務の主流（資金効率・流動性）。換算式が要る。""")
)
...
cells.append(
    md(r"""> **核心** — Δ は動くので、定期的に建玉を組み替える（リバランス）。<br>
> **直感** — 連続リバランスなら複製は完全。離散だとコストに分散（誤差）が残る。<br>
> **実務** — 『頻度↑で誤差↓・コスト↑』のトレードオフ最適化がヘッジ運用の核心。""")
)
...
cells.append(
    md(r"""> **核心** — 『ITM で買い・OTM で売る』素朴なストップロスは複製にならない。<br>
> **直感** — S が K 近傍を往復するたび高値買い・安値売りでコストが嵩み、観測頻度を上げても収束しない。<br>
> **実務** — 『直感的に正しそうなヘッジ』が破綻する好例。Greeks に基づく複製がなぜ要るかの反面教師。""")
)
...
cells.append(
    md(r"""> **核心** — 離散デルタヘッジのコストは BSM 価格を中心に分布し、頻度↑で締まる。<br>
> **直感** — 残差の分散は『Γ に対して (ΔS)² が暴れる』ことから生じる。<br>
> **実務** — プライシングは分布の平均、リスク管理は分布の幅。実デスクは両方を見る。""")
)
...
cells.append(
    md(r"""> **核心** — Γ は Δ の変化率＝ペイオフの曲がり。ロングオプションは Γ>0。<br>
> **直感** — Γ が大きいほど Δ が速く動き、リバランス頻度を上げないと複製が崩れる。<br>
> **実務** — Γ 中立化にはオプションが必要（原資産は Γ=0）。満期直前・ATM で Γ が暴れ最も危険。

> **実務での出番 — なぜ満期直前の ATM が最も危険か**
>
> Γ はストライク近傍・満期直前で急騰する。Δ が一瞬で 0↔1 を行き来するため、わずかな値動きで複製比率が激変し、リバランスが追いつかない。決算跨ぎの短期 ATM オプションのヘッジが現場で最も神経を使うのはこのため。満期で S≈K に張り付く『ピン・リスク』もここから生じる。""")
)
...
cells.append(
    md(r"""> **核心** — ロング Γ は『動けば得』だが、その代金として Θ（時間減価）を払う。<br>
> **直感** — Θ と Γ は符号が逆で表裏一体。BSM 方程式がこの均衡を表す。<br>
> **実務** — ガンマ・スキャルピング（動きで稼ぐ）vs 時間減価（払う）のバランスが日々の損益を決める。""")
)
...
cells.append(
    md(r"""> **核心** — 複数のオプションを使い Γ と ν を同時に 0 にできる。<br>
> **直感** — 連立方程式で必要数量を決め、最後に原資産で Δ を 0 に戻す。<br>
> **実務** — 中立化は『その瞬間だけ』。時間が経てば再調整が要る——ヘッジは終わりのない作業。""")
)
...
cells.append(
    md(r"""> **核心** — プットを『買う』代わりに、Δ ヘッジで合成（複製）できる。<br>
> **直感** — 連続取引できれば合成プット＝本物のプット。だが皆が同時に売ると前提が崩れる。<br>
> **実務** — 1987年の合成ポートフォリオ保険が暴落を増幅。摩擦なき連続取引という前提の危うさ。""")
)
