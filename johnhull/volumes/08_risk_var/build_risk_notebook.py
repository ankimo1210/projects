...
cells.append(
    md(r"""> **核心** — VaR＝『ある信頼水準での最大損失』。ES＝それを超えたときの平均損失。<br>
> **直感** — 損失分布の分位点が VaR、テール平均が ES。ES の方がテールを正しく測る。<br>
> **実務** — 銀行の規制資本・リスク限度の根幹。Basel は VaR99%→ES97.5% へ移行した。""")
)
...
cells.append(
    md(r"""> **核心** — VaR は『信頼水準 c・期間 N 日で、損失が VaR を超える確率は 1−c』。<br>
> **直感** — iid 正規なら N 日 VaR＝√N×1日 VaR。ボラ・クラスタリングがあると過小評価。<br>
> **実務** — リスク限度・資本計算の共通言語。期間と信頼水準の前提が命。""")
)
...
cells.append(
    md(r"""> **核心** — VaR は劣加法性を満たさない——分散したのにリスクが増えて見えることがある。<br>
> **直感** — 分位点はテールの形を無視する。ES はテール平均なので劣加法性を満たす。<br>
> **実務** — FRTB が ES へ移った主因の一つ。リスク尺度の選択は規制資本に直結。

> **実務での出番 — 劣加法性が壊れると何が困るか**
>
> リスク尺度が『分散効果でリスクは増えない』(劣加法性)を満たさないと、ポートフォリオを分割した方がリスクが小さく見える、という不合理が起きる。すると限度管理や資本配分が歪み、リスクを隠す動機すら生む。ES はこの性質を満たすため、危機後の Basel FRTB は市場リスク基準を VaR99% から ES97.5% へ切り替えた。""")
)
...
cells.append(
    md(r"""> **核心** — ポジションを正規・線形と仮定し、共分散行列から VaR を解析的に求める。<br>
> **直感** — VaR＝z×√(αᵀCα)。速いが、太い裾と非線形(オプション)に弱い。<br>
> **実務** — 高速な日次 VaR の定番だが、テールとガンマを取りこぼす限界を理解して使う。""")
)
...
cells.append(
    md(r"""> **核心** — 過去の変動をそのまま当てて損益分布を作る。分布形を仮定しない。<br>
> **直感** — 実際に起きた太い裾をそのまま拾える。過去に無い事象は出せないのが弱点。<br>
> **実務** — 最も広く使われる VaR 法。ストレス期間の選択が結果を左右する。""")
)
...
cells.append(
    md(r"""> **核心** — 危機期間で測る Stressed VaR＋超過回数で検証(バックテスト)。<br>
> **直感** — 99% VaR なら超過は年1%程度のはず。多すぎればモデル不良。<br>
> **実務** — Basel のバックテスト(トラフィックライト)で資本上乗せが決まる。モデル品質の番人。""")
)
...
cells.append(
    md(r"""> **核心** — オプションは非線形。デルタだけの線形近似は VaR を誤る。<br>
> **直感** — ロングオプションは凸性が守る → 線形近似は VaR を過大評価。デルタ-ガンマで補正。<br>
> **実務** — オプション在庫を持つデスクの VaR は非線形補正が必須。完全評価(フルリバル)が理想。""")
)
...
- **PCA**: 金利カーブの動きを「レベル・スロープ・曲率」少数因子に圧縮して VaR を効率化
  （旧 `interest_rate_models/ir_models.ipynb` のカーブパターンが因子の直感に対応）""")
)
...
cells.append(
    md(r"""---
*第8冊おわり。`hullkit.risk` は vol 27（risk desk）で再利用します。*""")
)
