...
> **直感** — 将来エクスポージャの期待値×デフォルト確率×損失率＝CVA。<br>
> **実務** — 危機後の必須調整。専門デスクがポートフォリオ横断で管理する。""")
)
...
cells.append(
    md(r"""> **核心** — EE(期待エクスポージャ)が CVA を駆動、PFE(潜在的将来エクスポージャ)が限度枠。<br>
> **直感** — 将来価値の分布の正の側。ボラ↑ で両者が広がる。<br>
> **実務** — 信用限度・担保・CVA の基礎量。MC で全取引の将来分布を作る。""")
)
...
cells.append(
    md(r"""> **核心** — 相手方信用(CVA)、自社信用(DVA)、資金コスト(FVA)の調整。<br>
> **直感** — それぞれ別の市場(CDS・調達)に紐づく。符号は立場による。<br>
> **実務** — 取引価格の構成要素。会計・規制・損益で扱いが分かれる論点。""")
)
...
cells.append(
    md(r"""> **核心** — 共通ファクターで多数名柄のデフォルトを結合。相関↑ でテールが厚く。<br>
> **直感** — 1つのマクロ要因への共通感応度が同時倒産を生む。<br>
> **実務** — ポートフォリオ信用損失・CDO の標準モデル。相関が損失分布の形を決める。

> **実務での出番 — 相関こそがシステミックリスク**
>
> 個々のデフォルト確率(pd)を固定しても、相関 ρ を上げるだけで損失分布の平均は不変なのにテールが急激に厚くなる。これは『多数の名柄に分散したから安全』という直感を裏切る——共通ショックがあれば分散は効かない。ポートフォリオ信用リスク、CDO、銀行の自己資本、すべての中心にこの相関がある。2008年の本質。""")
)
...
cells.append(
    md(r"""> **核心** — ガウスコピュラはテール依存がゼロ——極端な同時事象を過小評価する。<br>
> **直感** — 正規の裾は薄い。t コピュラ等は同時暴落を拾える。<br>
> **実務** — コピュラ選択がテールリスク評価を左右。モデルリスクの典型。""")
)
...
cells.append(
    md(r"""> **核心** — 無限粒度の極限で損失分布が閉形式に。Basel IRB の理論的背景。<br>
> **直感** — 共通ファクターの分位点が、ポートフォリオ損失の分位点になる。<br>
> **実務** — 銀行の規制資本(IRB)の数式そのもの。資本計算の根幹。""")
)
