...
cells.append(
    md(r"""> **核心** — 数理だけでなく、市場の歴史・失敗・実物への応用がデリバティブの全体像を作る。<br>
> **直感** — 同じリスク中立評価が証券化・実物投資・ESO にも貫通する。失敗例は仮定が崩れた事例。<br>
> **実務** — モデルを使う側の常識——どこで効き、どこで壊れるかを歴史から学ぶ。""")
)
...
cells.append(
    md(r"""> **核心** — 先物・オプション・スワップ・信用・エキゾチックは一枚の地図に収まる。<br>
> **直感** — すべて『将来のキャッシュフローをリスク中立で値付ける』派生形。<br>
> **実務** — シリーズ全巻の見取り図。どの道具をいつ使うかの全体像。""")
)
...
cells.append(
    md(r"""> **核心** — ローンを束ねトランシェ化する証券化が、2008年危機の中心にあった。<br>
> **直感** — 分散したつもりが、共通ショック(住宅全国下落)で相関が1へ → AAA まで毀損。<br>
> **実務** — モデルの相関仮定・インセンティブ設計の失敗。リスク管理の最重要教訓。

> **実務での出番 — 『分散』が効かなくなる瞬間**
>
> 証券化の安全神話は『デフォルトは独立に近い』という前提に乗っていた。だが共通ファクター(住宅価格・流動性)が効く危機では、相関が一斉に上がり分散効果が蒸発する。平時の相関でテールを測ると破局を見逃す——これは CDO に限らず、あらゆるポートフォリオ・リスク管理に通じる一般教訓。""")
)

# Cell 06: waterfall demo
cells.append(
    code(r"""# --- トランシェの損失ウォーターフォール（プール損失をエクイティから順に吸収） ---
# Hull GE Fig 8.1（p.202）: 元本 $100M = エクイティ $5M・メザニン $15M・シニア $80M
# → 損失の最初の 5% はエクイティ、5–20% はメザニン、20% 超はシニアが負担（p.203）
tranches = [("エクイティ", 0.0, 0.05), ("メザニン", 0.05, 0.20), ("シニア", 0.20, 1.0)]
...
**ABS CDO の上位**が AAA を名乗る。Hull Fig 8.3 の例（ABS CDO をシニア 65%・メザニン 25%・エクイティ 10% に分割）では、
原資産の損失が 10.25% を超えるだけで ABS CDO の AAA トランシェが毀損し、17% で 69.2% を失う（Table 8.1、検証セルで再現）。
ABS 自体の AAA トランシェ（20% 超で毀損）よりはるかに脆い。だが原資産は同じサブプライムの相関リスクで、
...
- 新株発行の**希薄化**: 効率的市場では付与公表時に株価へ織り込み済みで、現在株価で評価した BSM 値に再度掛ける必要はない（Hull の留保）。$N/(N+M)$ はワラント・ペイオフの乗数として登場する""")
)
cells.append(
    md(r"""> **核心** — ESO は早期行使・権利確定・希薄化で通常オプションと異なる。<br>
> **直感** — 従業員は分散できず早期行使しがち → 実効満期が短い。<br>
> **実務** — 費用計上(会計)・報酬設計の実務。BSM の単純適用は過大評価になり得る。""")
)
...
# 希薄化の図解: ワラント型ペイオフの乗数 N/(N+M)（効率的市場では株価に織込み済み）
N_shares, M_options = 10_000_000, 1_000_000
dilution = N_shares / (N_shares + M_options)
print(f"\n参考: ワラント乗数 N/(N+M) = {dilution:.4f}")
print("※ Hull: 効率的市場では希薄化は付与公表時に株価へ反映済み → 通常は再調整不要")""")
)
...
cells.append(
    md(r"""> **核心** — コモディティは平均回帰・季節性・保管費という固有の性質を持つ。<br>
> **直感** — 価格は均衡へ引き戻される(Schwartz 一要因)。在庫・天候が効く。<br>
> **実務** — 電力・ガス・原油のヘッジと値付け。金融資産と違う動学に注意。""")
)
...
cells.append(
    md(r"""> **核心** — 事業の意思決定(拡張・撤退・延期)をオプションとして評価する。<br>
> **直感** — 拡張＝コール、撤退＝プット。不確実性が高いほど『待つ価値』が増す。<br>
> **実務** — 設備投資・資源開発・R&D の評価。NPV では捨てる柔軟性の価値を拾う。""")
)
...
| Kidder Peabody（Jett） | 会計システムの欠陥が架空利益を生成 |
...
cells.append(
    md(r"""> **核心** — Barings・LTCM・Metallgesellschaft——失敗には共通パターンがある。<br>
> **直感** — レバレッジ・流動性・『市場を出し抜ける』過信。ヘッジが投機に化ける。<br>
> **実務** — ヘッジの目的はリスク削減であって収益向上ではない、という規律。

> **実務での出番 — LTCM と流動性ブラックホール**
>
> ノーベル賞級の頭脳が組んだ収束裁定は、平時は機能した。だが1998年、皆が同じポジションを持つ中で危機が起き、誰もが同時に手仕舞おうとして流動性が消滅——価格は『理論値へ収束』どころか発散した。教訓：モデルが正しくても、流動性とポジションの混雑(crowding)を無視すると破綻する。『市場を継続的に出し抜ける』前提を置かないことが最大の防御。""")
)
...
checks.append(("放棄オプション 8.3427",
               ...
               8.3427, 1e-3))
checks.append(("トランシェ境界 = Hull Fig 8.1（5% / 20%）",
               float([(lo, hi) for _, lo, hi in tranches] == [(0.0, 0.05), (0.05, 0.20), (0.20, 1.0)]),
               1.0, 0.0))
checks.append(("ウォーターフォール: プール12%でメザニン損失7%", allocate_loss(0.12, 0.05, 0.20), 0.07, 1e-12))
checks.append(("ウォーターフォール: プール12%でメザニン毀損率 7/15",
               allocate_loss(0.12, 0.05, 0.20) / 0.15, 7.0 / 15.0, 1e-12))
checks.append(("ウォーターフォール: プール12%でシニア無傷", allocate_loss(0.12, 0.20, 1.0), 0.0, 1e-12))
checks.append(("ウォーターフォール: プール25%でシニア損失5%", allocate_loss(0.25, 0.20, 1.0), 0.05, 1e-12))
# Hull GE Table 8.1（p.205）: ABS メザニン（5–20%）の損失率と、それを 10/25/65% に再トランシェ化した ABS CDO
for pool_l, want_mezz, want_cdo_mezz, want_cdo_senior in ((0.10, 0.333, 0.933, 0.0), (0.13, 0.533, 1.0, 0.282),
                                                          (0.17, 0.800, 1.0, 0.692), (0.20, 1.0, 1.0, 1.0)):
    mezz_l = allocate_loss(pool_l, 0.05, 0.20) / 0.15
    checks.append((f"Table 8.1 損失{pool_l:.0%}: ABS メザニン {want_mezz:.1%}", mezz_l, want_mezz, 5e-4))
    checks.append((f"Table 8.1 損失{pool_l:.0%}: CDO メザニン {want_cdo_mezz:.1%}",
                   allocate_loss(mezz_l, 0.10, 0.35) / 0.25, want_cdo_mezz, 5e-4))
    checks.append((f"Table 8.1 損失{pool_l:.0%}: CDO シニア {want_cdo_senior:.1%}",
                   allocate_loss(mezz_l, 0.35, 1.0) / 0.65, want_cdo_senior, 5e-4))
