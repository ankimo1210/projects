...
cells.append(
    md(r"""> **核心** — 市場は BSM の『σ 一定』を信じていない。スマイルがその証拠。<br>
> **直感** — 行使価格ごとに違う IV ＝ 市場が想定する分布が対数正規でない、ということ。<br>
> **実務** — 正しいスマイルの把握はエキゾチック値付け・ヘッジ・リスク計測の前提。""")
)
...
cells.append(
    md(r"""> **核心** — IV＝市場価格を BSM に逆算して得る『市場の言葉』。<br>
> **直感** — put-call parity から、同じ K のコール/プットの IV は一致。スマイルは1本の曲線。<br>
> **実務** — オプションは価格でなく IV で気配・取引される。IV が共通通貨。""")
)
...
cells.append(
    md(r"""> **核心** — 株式は左に歪んだスキュー、FX は両裾の厚いスマイル。<br>
> **直感** — 株は暴落(左裾)が怖い → 左の IV が高い。FX は両方向のジャンプ → 両裾が高い。<br>
> **実務** — 原資産クラスでヘッジ・在庫の偏りが違う。スキューはクラッシュ保険の値段。""")
)
...
cells.append(
    md(r"""> **核心** — IV は行使価格×満期の2次元の面(サーフェス)。<br>
> **直感** — 満期が延びるとスマイルは平坦化。ATM 断面がタームストラクチャー。<br>
> **実務** — デスクは面全体を管理。面の補間・無裁定性がエキゾチック値付けの基盤。""")
)
...
cells.append(
    md(r"""> **核心** — コール価格を K で2回微分すると、リスク中立密度が出る。<br>
> **直感** — オプション価格 ⇔ 市場が織り込む確率分布、は表裏一体。スキュー → 左裾の厚い分布。<br>
> **実務** — 市場が織り込むテール確率を直接読める。テールヘッジ・リスク評価に使う。""")
)
...
> **直感** — sticky strike か sticky delta かでデルタが変わる。規約を決めないとヘッジがぶれる。<br>
> **実務** — デスクのヘッジ規約は P&L 帰属に直結する実務上の重要決定。""")
)
...
cells.append(
    md(r"""> **核心** — 過去リターンからボラを推定。EWMA は新しいデータを重く見る。<br>
> **直感** — 単純窓は古い大変動が抜ける瞬間に不自然なジャンプ。指数加重で滑らかになる。<br>
> **実務** — リスク計測(VaR)の日次ボラ更新。RiskMetrics の EWMA が業界の出発点。""")
)
...
cells.append(
    md(r"""> **核心** — ボラは平均回帰し、クラスタリングする(荒れた日が固まる)。<br>
> **直感** — 今日の分散＝長期水準＋直近ショック＋直近分散の加重。α+β<1 で定常。<br>
> **実務** — ボラ予測・オプション値付け・VaR のボラ動学モデルの標準。

> **実務での出番 — ボラティリティ・クラスタリングはなぜ重要か**
>
> 市場の大変動は固まって起きる(2008、2020)。これを無視した一定ボラ仮定は危機時の VaR を系統的に過小評価する。GARCH/EWMA は直近の荒れを取り込みボラを引き上げる。Basel が『ストレス期間』でのカリブレーションを求めるのも、平時のボラでテールを測る危うさへの対応。""")
)
...
cells.append(
    md(r"""> **核心** — 観測リターンの尤度を最大化して GARCH パラメータを決める。<br>
> **直感** — 『このパラメータならこのデータが出やすい』を最大化。バリアンス・ターゲティングで安定化。<br>
> **実務** — モデルの実データへの当てはめ。推定の安定性が予測の信頼性を左右する。""")
)
...
cells.append(
    md(r"""> **核心** — GARCH から将来ボラの期間構造を予測できる。<br>
> **直感** — 出発が高/低ボラでも、予測は長期水準へ平均回帰していく。<br>
> **実務** — 満期別オプションの値付け、ボラ・スワップの期間構造に直結。""")
)
...
cells.append(
    md(r"""> **核心** — 多資産では相関も推定が要る。Cholesky 分解で相関付き乱数を作る。<br>
> **直感** — Σ=LLᵀ、X=LZ で独立乱数から相関構造を注入する。<br>
> **実務** — ポートフォリオ VaR・多資産 MC の必須部品。相関は危機時に1へ寄る点に注意。""")
)
