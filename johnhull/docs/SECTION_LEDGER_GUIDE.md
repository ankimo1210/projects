...
M3aで[§26.11のL01–L06と独立128価格](SECTION_26_11_REVIEW_2026-09-15.md)を確認し、現在は受入済み2、不足あり1、未評価303項目。
過去の実装分類をそのまま学習内容の受入へ移していない。
全節の現在の充足率・計算カバレッジは、この台帳だけではまだ確定できない。

M2bは教材・図・Book/portalを実際に検査し、共有sourceを変更した§26.9も再検証した。
古いdouble-checkは履歴として保持し、現在の台帳はm2b-recheckを参照する。
