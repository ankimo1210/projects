...
納品時点（独立レビュー前）の記録。VS01–VS06 の独立参照・公開 API・教材 6 小節・共有 4 図・
Book/portal の実画面確認まで揃えた段階で、台帳は pending_validation だった。
その後の独立レビューの指摘と対応は [FEEDBACK](SECTION_26_16_FEEDBACK_2026-09-25.md)、
受入の判断は [ACCEPTANCE](SECTION_26_16_ACCEPTANCE_2026-09-25.md) にある。以下の数値と記述は
レビュー対応を反映した現行版。
...
- 式 26.6 を $x=\ln(K/F)$ の有限区間で `quad` 積分。平坦 BSM は $\sigma^2$、Heston は
  ...
  Lewis 一重積分。深い OTM では約 $10^{-11}$ の相殺ノイズが出る（`strip_min_q` = −2.06e-11）。
  負になった価格の $E(V)$ での重みは合計でも最大 2.4e-14（`strip_negative_q_max_weight`）。
...
| Heston 3 市場の式 26.6 | 閉形式との最大差 1.51e-12（long-uncorrelated 市場。図に描いた skewed 市場では 2.9e-15） |
...
| 式 26.9、ξ=0.3 / 1.0 | 近似誤差 −1.9e-5 / −4.6e-3、素朴な $\sqrt{E(V)}$ は +2.6e-3 / +2.5e-2。隣接点の対数傾きは 4.1–4.7（小さい ξ で ξ⁴） |
| MC（ξ=0.3/0.6/0.9） | $E(\sqrt V)$ の最大 1.08 SE |
| 日次推定量（GBM、n=64、σ=25%） | $n-2$ の期待値 0.063509（$\sigma^2$ に対し +0.00101）、$n-1$ は +5.0e-7。MC（分母ごとに別の乱数、20 万本）は −1.13 / +0.18 SE |
...
- notebook：115 セル、§4.6 の外の 101 セルはソースと出力 53 件（実行番号を除き、Plotly の div UUID は伏せる）が
  基準コミット f6a2b62e と一致。出力の 1 文字改変・§26.16 図の数値改変・§26.15 図の数値改変はいずれも拒否された。
...
- ブラウザー：Chromium 145、Book/portal × 1440/1000 px × 4 図 × 2 状態 = 32 状態を独立参照
  ...
  ξ=0.6 の式 26.9 の値を +0.5 %pt 改変すると両面で拒否され、元に戻る。Book の MathJax 112 式、エラー 0。
  画像は 4 図 × 両面 × 2 幅の 16 枚と、Book の §4.6.4（式 26.9・Example 26.5）と §4.6.5（式 26.10・VIX）の各 2 幅。
...
- 実現分散の MC は分母ごとに別の乱数で回した（同じ乱数だと両者は定数倍で、検査が 1 つになる）。
