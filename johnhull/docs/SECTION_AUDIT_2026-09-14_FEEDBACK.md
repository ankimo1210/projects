...
## 最新追記：M4b §26.12の独立価格・教材・配布検査（2026-09-16）

[受入ノート](SECTION_26_12_ACCEPTANCE_2026-09-16.md)と[統合記録](validation/section-26-12/m4b-check.json)にS01–S06を集約した。
独立42価格を固定したまま別実装のCRRで照合し、N1024の最大絶対残差は0.003793。本文6小節・共有4図をBook/portalへ接続した。
新旧4節の回帰は622件成功・6スキップ。新節は13状態×両面×2幅＝52状態で数値・配置を検査し、符号付き脚の誤配分も拒否した。
Task1–3の独立レビューは承認済み。全体テストと最終ブランチ判定は統合記録に記載する。台帳は受入4・不足0・未評価302。

今回の抜けは、必須ハッシュ自体の欠落、数値精度から境界精度への誤った連想、凡例・操作バーの表示衝突だった。
欠落の改変検査、実ノード幅と独立B境界の区別、hover状態の全メニュー検査を追加した。
本文の要求番号と節の読む順序も区別し、台帳は比較を§4.2.5、適用域を§4.2.6へ対応させた。
M4aのlookback比較に関する広い記述は履歴として保持するが、今回の正しい実測範囲はr=qの6行を除く36行・6市場である。
独立二方式の相互比較はATM14条件。42価格すべてで二方式が一致したという主張ではない。

## M3b時点：§26.11の教材・図・配布検査（2026-09-16）
