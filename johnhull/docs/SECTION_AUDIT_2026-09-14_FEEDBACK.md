...
## 最新追記：M3b §26.11の教材・図・配布検査（2026-09-16）

[受入ノート](SECTION_26_11_ACCEPTANCE_2026-09-16.md)へ、L01–L06の本文6小節・共有4図・独立検証を接続した。
Book/portalの各面・各幅（1440/1000）で全11状態のレイアウトを検査。各面で経路内部48点の極値とcall/put複製の符号付きlegを独立照合した。
lookback 144・binary 92・barrier 171の計407テストに加え、全体1,695テストも成功した（79.36秒、既存警告2件）。Task 2独立レビューは承認済み。最終ブランチレビューも承認済みで、結果は[統合記録](validation/section-26-11/m3b-check.json)に集約する。
台帳は受入済み3、不足あり0、未評価303。r=q近傍の価格APIは未対応のまま、適用域と可除特異点を教材で説明する。

今回も「サンプル点の値が正しい」だけでは図の意味を保証できなかった。
独立レビューで、節点のrunning extremaを直線で結ぶと交差前から極値が動く問題を発見し、交差時点を図へ追加した。
節点間の独立基準点と、総額を保つ誤配分の改変検査が、見かけの整合性だけでは通らない条件になった。
これを次節でも、要求・数値・図の意味・実画面それぞれの検査へ落とし込む。

## M3a時点：§26.11の要求と独立検証（2026-09-15）
