"""Rerun the accepted §26.9–§26.17 test suites on the M10 build and write m10-recheck.json.

M10 (§26.17) changed the shared vol10 notebook and builder, the portal registry and
...
commit) next to a fresh run of each section's own tests. Historical M8 records
are left untouched.
"""
...
def _sources(section):
    record = PROJECT / f"docs/validation/section-{section}/m8-recheck.json"
...
def recheck(section, base):
    ...
    lesson = (
        PROJECT
        / f"docs/validation/section-{section}/{'reference.json' if section == '26-17' else 'lesson-data.json'}"
    )
    unchanged = None
    if lesson.is_file():
    ...
    return {
        ...
        "scope": (
            "Accepted section tests against the final M10 assets after the shared "
...
def main():
    ...
    parser.add_argument("--base", default="7cfd0864")
    parser.add_argument("sections", nargs="*", default=list(TESTS))
    options = parser.parse_args()
    failed = []
    for section in options.sections:
        record = recheck(section, options.base)
        path = PROJECT / f"docs/validation/section-{section}/m10-recheck.json"
