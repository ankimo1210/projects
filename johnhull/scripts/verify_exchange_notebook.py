"""Check the vol10 notebook after the §26.14 insertion and write the M6b record.
...
outside the rewritten §4.4-§4.5 span preserved against the base commit.
"""
...
OUTPUT = PROJECT / "docs/validation/section-26-14/notebook-m6b-check.json"
SOURCE_PATHS = (
    NOTEBOOK,
    BUILDER,
    "hullkit/src/hullkit/_exchange_lesson.py",
    "hullkit/src/hullkit/exotics.py",
    "docs/validation/section-26-14/lesson-data.json",
    "scripts/verify_exchange_notebook.py",
)
PLOTLY_MIME = "application/vnd.plotly.v1+json"
# The span this milestone rewrote: the old exchange demo and rainbow cell became
# sections 4.4 and 4.5. Everything before and after must be byte-identical.
START_MARKERS = ("### 4.4 交換オプション", "交換オプション（Margrabe）")
...
def saved_figures(notebook):
    """Every Plotly payload the notebook stores for a §26.14 figure."""
    found = {}
...
def compare_figures(saved):
    ...
    from hullkit._exchange_lesson import _figures
...
def preservation(reference, notebook):
    """Cells outside the rewritten §4.4-§4.5 span must be identical to the base commit."""
    ...
    def outside(book):
        ...
        for cell in book.cells:
            ...
            if any(marker in source for marker in START_MARKERS) or source.startswith("#### 4.4"):
                inside = True
            elif source.startswith("### 4.5") or "レインボー・オプション" in source:
                inside = True
            elif END_MARKER in source:
                inside = False
            elif inside and cell.cell_type == "code" and "exchange" in source:
                continue
            if not inside:
    ...
    if kept_before != kept_after:
        for index, (old, new) in enumerate(zip(kept_before, kept_after, strict=False)):
            if old != new:
                raise ValueError(f"cell {index} outside §4.4-4.5 changed: {old[1][:60]!r}")
        raise ValueError(
            f"cell count outside §4.4-4.5 changed: {len(kept_before)} -> {len(kept_after)}"
        )
    return {
...
def main():
    ...
    parser.add_argument("--base", default="42a6cbc0")
    ...
    record = {
        ...
        "saved_plotly_note": (
            "Each saved exchange payload equals hullkit._exchange_lesson._figures() exactly, "
            "data and layout."
        ),
