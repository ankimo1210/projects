"""Rerun the accepted §26.9–§26.17 and §27.1 test suites on the M11 build.

M11 (§27.2) changed the shared vol06 notebook and builder, the portal registry
and stylesheet, the release manifest and the READMEs, whose hashes the accepted
...
commit) next to a fresh run of each section's own tests and writes
m11-recheck.json. Historical records are left untouched.
"""
...
TESTS = {
    ...
    "26-16": [
        ...
        "hullkit/tests/test_variance_swap_contracts.py",
        "hullkit/tests/test_variance_swap_lesson.py",
    ],
    "26-17": [
...
def _sources(section):
    record = PROJECT / f"docs/validation/section-{section}/m10-recheck.json"
...
def recheck(section, base):
    ...
    lesson = (
        PROJECT
        / f"docs/validation/section-{section}/{'reference.json' if section in ('26-17', '27-1') else 'lesson-data.json'}"
    )
    unchanged = None
    if lesson.is_file():
        unchanged = _base_bytes(base, f"docs/validation/section-{section}/{lesson.name}") == (
            lesson.read_bytes()
        )
    ...
    return {
        ...
        "scope": (
            "Accepted section tests against the final M11 assets after the shared "
...
def main():
    ...
    parser.add_argument("--base", default="c3dd6ae5")
    parser.add_argument("sections", nargs="*", default=list(TESTS))
    options = parser.parse_args()
    failed = []
    for section in options.sections:
        record = recheck(section, options.base)
        path = PROJECT / f"docs/validation/section-{section}/m11-recheck.json"
