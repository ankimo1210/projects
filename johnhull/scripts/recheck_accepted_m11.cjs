// Run the accepted §26.9–§26.17 and §27.1 browser verifiers on M11 builds without replacing historical evidence.
...
const verifiers = {
  ...
  '26-16': 'verify_variance_swap_lesson_browser.cjs',
  '26-17': 'verify_static_replication_browser.cjs',
  '27-1': 'verify_alternative_models_browser.cjs',
};
...
if (!section) {
  ...
  const transformed = original.replace(/\.png(?=['"`])/g, '-m11-recheck.png')
    .replace(/browser-(?:m\d+b?-)?check\.json/g, 'browser-m11-recheck.json');
  const digest = text => crypto.createHash('sha256').update(text).digest('hex');
  const write = fs.writeFileSync;
  fs.writeFileSync = function (file, data, ...args) {
    if (String(file).endsWith('/browser-m11-recheck.json')) {
      const record = JSON.parse(data);
      record.command = 'CHROMIUM_BIN=<existing runtime> PLAYWRIGHT_MODULE=<existing runtime> node scripts/recheck_accepted_m11.cjs ' + section;
      record.recheck = {
        milestone: 'M11', original_verifier: 'scripts/' + verifiers[section],
      ...
      for (const image of fs.readdirSync(path.dirname(file)).filter(n => n.endsWith('-m11-recheck.png'))) {
