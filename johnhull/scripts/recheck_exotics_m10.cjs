// Run the accepted exotics verifiers on M10 builds without replacing historical evidence.
...
if (!section) {
  ...
  const transformed = original.replace(/\.png(?=['"`])/g, '-m10-recheck.png')
    .replace(/browser-(?:m\d+b?-)?check\.json/g, 'browser-m10-recheck.json');
  const digest = text => crypto.createHash('sha256').update(text).digest('hex');
  const write = fs.writeFileSync;
  fs.writeFileSync = function (file, data, ...args) {
    if (String(file).endsWith('/browser-m10-recheck.json')) {
      const record = JSON.parse(data);
      record.command = 'CHROMIUM_BIN=<existing runtime> PLAYWRIGHT_MODULE=<existing runtime> node scripts/recheck_exotics_m10.cjs ' + section;
      record.recheck = {
        milestone: 'M10', original_verifier: 'scripts/' + verifiers[section],
      ...
      for (const image of fs.readdirSync(path.dirname(file)).filter(n => n.endsWith('-m10-recheck.png'))) {
