"""Rerun the accepted §26.9–§26.16 test suites on the M9 build and write m9-recheck.json.

M9 (§26.17) changed the shared vol10 notebook and builder, the portal registry and
...
def recheck(section, base):
    ...
    return {
        ...
        "scope": (
            "Accepted section tests against the final M9 assets after the shared vol10 notebook, "
...
def main():
    ...
    parser.add_argument("--base", default="f0d25b9e")
    parser.add_argument("sections", nargs="*", default=list(TESTS))
    options = parser.parse_args()
    failed = []
    for section in options.sections:
        record = recheck(section, options.base)
        path = PROJECT / f"docs/validation/section-{section}/m9-recheck.json"
