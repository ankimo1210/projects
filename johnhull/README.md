...
11e core covers all 37 chapters; volumes 18--26 add ML surrogates, inverse
volatility surfaces, surface dynamics, SPX/VIX and 0DTE, post-LIBOR RFRs,
crypto market structure, climate/energy derivatives, and inflation-linked rates/JGBi. See `ROADMAP.md` for
...
- `volumes/18_*` … `volumes/26_*` — artifact-only beyond-Hull teaching notebooks
...
`codex/johnhull-beyond-hull-g8` are recorded in `VALIDATION.md`. Volume 26 is
complete through its Phase 7 artifact/notebook gate; portal, book, and the full
tracked release remain explicitly deferred.

Run release checks from the workspace root:

```bash
make hull-artifacts-check  # rebuild vol. 19–26 in /tmp and compare references
