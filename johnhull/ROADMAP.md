...
| 節カバレッジ | [節別台帳](docs/SECTION_LEDGER.md)へ306項目を登録。§26.9–§26.12の4項目受入、§26.13はレビュー指摘対応済み・再レビュー待ち、301項目未評価。最終統合判定は各受入ノートと統合記録を参照。完了率は確定値として使わない |
...
| M5 | §26.13の要求抽出・独立参照・既存近似の誤差測定から教材・図・実画面まで | M5a／M5b実装・配布検査済み。独立レビューのP2 6件とP3 1件に対応（F1–F7）、再レビュー待ちでpending_validationを維持。[レビュー結果](docs/SECTION_26_13_FEEDBACK_2026-09-16.md)・[対応](docs/SECTION_26_13_ACCEPTANCE_2026-09-16.md) |
