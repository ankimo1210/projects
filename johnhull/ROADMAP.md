...
| 節カバレッジ | [節別台帳](docs/SECTION_LEDGER.md)へ306項目を登録。§26.9–§26.12の4項目受入、§26.13は不足あり、301項目未評価。M4bの最終統合判定は受入ノートと統合記録を参照。完了率は確定値として使わない |
...
| M5 | §26.13の要求抽出・独立参照・既存近似の誤差測定から教材・図・実画面まで | M5a完了：A01–A06と144参照価格、モーメント整合の誤差を測定（+23.35%〜−6.85%）。gaps_found。M5bの実装・教材・図は未着手 |
...
現在地は[§26.13のM5a](docs/SECTION_26_13_REVIEW_2026-09-16.md)。台帳は受入4・不足あり1・未評価301。
既存の`asian_call_turnbull_wakeman`は連続平均・新規発行・callの1本だけで、離散観測・既発契約・平均行使型は未実装。
モーメント整合の近似誤差を144行で初めて測り、+23.35%〜−6.85%（Example 26.3自体が約+1%高い）と分かった。
M5bは離散モーメント・K*変換・誤差の明示を、その[受入条件](docs/SECTION_26_13_REVIEW_2026-09-16.md)8項目に沿って実装する。
...
portalは94図、vol10は74セル。putは原典外の独立拡張、r=qのlookbackは既存API未対応として欠測表示する。
§26.13の要求抽出と独立基準の確認はM5aで完了した。既存Asian実装の存在を受入判定へ流用していない。
