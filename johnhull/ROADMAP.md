...
| 節カバレッジ | [節別台帳](docs/SECTION_LEDGER.md)へ306項目を登録。§26.9–§26.15の7項目受入、299項目未評価。最終統合判定は各受入ノートと統合記録を参照。完了率は確定値として使わない |
...
現在地（2026-09-19）は[§26.15のM7](docs/SECTION_26_15_ACCEPTANCE_2026-09-19.md)。台帳は受入7・gaps_found 0・未評価299。
独立72価格（48市場行＋24解析アンカー）に対し、公開API 2関数・本文6小節・共有4図を実装した。
Book/portal両面の1440/1000px・全10メニュー状態（計40状態）を独立参照と照合し、18画像を保存した。
近似誤差は全72行の最大絶対値6.4174通貨、参照価格0.5以上の63行では−1.94%〜+66.84%。
これは選んだ合成市場の経験的範囲であり、全域の精度保証ではない。
...
次は§26.16 Volatility and Variance Swapsを、既存教材の存在とは別に原典要求から監査する。
