...
| 節カバレッジ | [節別台帳](docs/SECTION_LEDGER.md)へ306項目を登録（M1）。§26.9を受入済みとし、残り305項目の内容は未評価。完了率は確定値として使わない |
...
| M2 | 次の1節を原典の要求から説明・実装・独立検証・図・実画面まで確認 | 未着手、対象節は未決定 |
| 以降 | 台帳を使って全節へ段階的に展開 | 未着手 |

M1のPASSは台帳と保存証跡の整合性を表す。数値モデルの再検証や全節の完成判定ではない。
最新の実行結果は [M1記録](docs/validation/section-ledger-m1/validation.json)。コミット・push・公開は未実施。
