...
| 節カバレッジ | [節別台帳](docs/SECTION_LEDGER.md)へ306項目を登録。§26.9–§26.12の4項目受入、§26.13は検証済み・独立レビュー待ち、301項目未評価。最終統合判定は各受入ノートと統合記録を参照。完了率は確定値として使わない |
...
| M5 | §26.13の要求抽出・独立参照・既存近似の誤差測定から教材・図・実画面まで | M5a／M5b完了：A01–A06、144参照価格、誤差測定（+23.35%〜−6.85%）、離散モーメント・K*変換・平均行使型の実装、本文6小節・4図、Book/portal両面2幅の検査。独立レビュー待ちでpending_validation |
...
現在地は[§26.13のM5b](docs/SECTION_26_13_ACCEPTANCE_2026-09-16.md)と
[統合記録](docs/validation/section-26-13/m5b-check.json)。台帳は受入4・検証済み1・未評価301。
...
モーメント整合の近似誤差は144行で+23.35%〜−6.85%（Example 26.3自体が+0.99%高い）。
...
残る受入条件は8項目めの独立レビューだけで、それまでacceptedへは変更しない。
...
portalは98図、vol10は83セル。putは原典外の独立拡張、r=qのlookbackは既存API未対応として欠測表示する。
