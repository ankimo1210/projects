...
| 節カバレッジ | [節別台帳](docs/SECTION_LEDGER.md)へ306項目を登録。§26.9–§26.15の7項目受入、§26.16は検証待ち（独立レビュー待ち）、298項目未評価。最終統合判定は各受入ノートと統合記録を参照。完了率は確定値として使わない |
...
| M8 | §26.16 Volatility and Variance Swaps（pp.629–632）の独立参照から教材・図・実画面まで | 納品済み、pending_validation（独立レビュー待ち）。印刷例26.4/26.5を独立再計算で再現、公開API 4関数、本文6小節、共有4図、Book/portal両面2幅・32状態。既受入7節も最終ビルドで再検証。[レビュー](docs/SECTION_26_16_REVIEW_2026-09-25.md) |
| 以降 | 台帳を使って残る未評価節（§26.17ほか）へ段階的に展開 | 未着手 |
...
現在地（2026-09-25）は[§26.16のM8](docs/SECTION_26_16_REVIEW_2026-09-25.md)。台帳は受入7・pending_validation 1・未評価298。
...
独立レビューが未実施のため、レビューか利用者の明示的な免除があるまでacceptedにしない。
...
§26.16 Volatility and Variance Swaps（pp.629–632）はM8でVS01–VS06の全軸を揃えたが、独立レビュー待ちでpending_validation。
