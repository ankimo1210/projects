...
make gpu-sanitize      # Compute Sanitizer memcheck
...
Makefile の既定は `/usr/local/cuda-12.9/bin/nvcc` と `-arch=sm_120`。
別環境では `CUDA_HOME` と `GPU_ARCH` を make の command line から上書きできる。

`make gpu-sanitize` は Compute Sanitizer memcheck を呼ぶ。
当初は test binary だけでなく `/bin/true` も `Error launching target app`、exit 13 になった。
system-call trace では、Windows 側を指す `TEMP` / `TMP` の DrvFS 上に一時 FIFO を作る `mknodat` が `EOPNOTSUPP` で失敗していた。
Makefile は sanitizer 実行時だけ `TMPDIR` / `TMP` / `TEMP` を Linux 側 `/tmp` へ固定し、この環境依存を局所化する。
修正後は memcheck・initcheck・synccheck が 66 assertions と 0 errors、racecheck が 0 hazards で完了した。
