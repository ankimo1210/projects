...
| `make plot` | リポジトリルートの uv 環境（pandas/matplotlib）で全 14 枚の PNG（ソート 6 + 検索 5 + 並列 3）を生成 |
...
```
cpp_algo_lab/
├── Makefile              # test / bench（sorting・search・parallel）/ trace / plot / clean
├── common/
...
│   ├── plots/            # 14 枚の PNG（ソート 6 + 検索 5 + 並列 3）
│   └── traces/           # アルゴリズム別スナップショット CSV
├── docs/                 # 学習ドキュメント（sorting.md / search.md / parallel_cpu.md）
