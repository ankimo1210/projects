...
> **状態: 全マイルストーン完了 + レベルアップ**。データ → リサーチ(features / **6信号ファミリ** /
> labels)→ 評価エンジン(walk-forward backtest)→ **モデル Tier0-4**(baseline/linear/tree/MLP/
> 時系列+Chronos アダプタ)→ ポートフォリオ → **基軸通貨 FX 換算** → 可視化/HTML → **日本税(NISA・配当)**。
...
>
> **レベルアップ層**(`backtest`/`risk`/`regime`/`diagnostics`/`models`/`visualization` に追加):
> **検証の厳格化**(Purged Combinatorial CV + MDA 重要度)、**カバレッジ拡充**(リスク分解 Euler/PCA・
> 因果レジーム検出・IC decay/ファクター露出/capacity)、**モデル強化**(アンサンブル/スタッキング・
> learning-to-rank・分位点回帰/conformal 区間)、**可視化の格上げ**(フル tearsheet・マルチ戦略
> ダッシュボード・ファクター heatmap・対話的パラメータ探索)。
>
...
quantkit/
  ...
  src/quantkit/   data/ macro/ features/ signals/ labels/ backtest/ models/ portfolio/ tax/ visualization/ utils/
                  risk/ regime/ diagnostics/   ← レベルアップ層
             ← すべて実装済み
...
→ **レベルアップ層**: `14_validation_rigor_and_diagnostics`(CPCV・MDA・IC decay・露出・capacity)
→ `15_risk_decomposition_and_regimes`(Euler/PCA リスク分解・因果レジーム)
→ `16_advanced_models_and_reporting`(ensemble/stacking・LTR・quantile/conformal・tearsheet/dashboard)。
...
```bash
uv run pytest quantkit/tests -q          # 159 passed, 7 skipped(ネット不要)
...
- **Phase 3(完了)— レベルアップ**:分析カバレッジ・モデル・可視化を一段引き上げ。
  - **検証の厳格化**(`backtest`):`combinatorial_purged`(López de Prado CPCV、two-sided purge +
    embargo、`is_purged`/`n_combinatorial_folds`)、`models.mda_importance`(OOS の MDA 重要度、
    既定スコアは横断 rank IC、`random_state` で決定的)。
  - **カバレッジ拡充**:`risk`(Euler 分解 `portfolio_vol`/`marginal_risk`/`component_risk`・PCA
    リスク因子 `pca_factors`/`effective_n_bets`)、`regime`(**因果**な `vol_regime`(expanding 分位)・
    `trend_regime`・`regime_summary`)、`diagnostics`(`ic_decay`・`factor_exposure`・`capacity`)。
  - **モデル強化**(`models`):`EnsembleModel`/`StackingModel`(OOF メタ特徴)、`learning_to_rank`/
    `cross_sectional_rank`、`quantile_gbm`・`ConformalModel`(分割コンフォーマル、有限標本 1-α 被覆)。
  - **可視化の格上げ**(`visualization`):`tearsheet`・`comparison_dashboard`(オフライン HTML)、
    `monthly_returns_heatmap`・`risk_return_scatter`・`factor_heatmap`・`parameter_explorer`(対話的)。
- **任意の後続(検証/深掘り)** — 鍵が要るソースのライブ確認(キー入手後)、EDINET の XBRL から
  財務数値抽出、BoJ/MoF の実 CSV 書式追従、Chronos の GPU 実行。(レベルアップ層の NB 14–16 は実走済。)
