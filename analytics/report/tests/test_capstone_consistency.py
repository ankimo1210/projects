...
all three books, so it is the natural home for this check).

The capstone teaches that one regression problem has three faces. This test
pins that down: the three books generate IDENTICAL data, and ridge (linear
algebra) == Bayesian posterior mean == gradient-descent-with-weight-decay (the
NN lens) on the shared features.
"""
...
def test_three_books_share_identical_data():
    a, b, c = la_data(seed=0), nn_data(seed=0), by_data(seed=0)
    for arr_a, arr_b, arr_c in zip(a, b, c, strict=True):
        np.testing.assert_array_equal(arr_a, arr_b)
        np.testing.assert_array_equal(arr_a, arr_c)


def test_three_lenses_agree():
    x, y = la_data(seed=0)
