...
| `09_spectral_nonlinear_pdes` | 発展: スペクトル法・KdV ソリトン・反応拡散(Turing)・2D 波動 |
| `10_capstone_three_lenses` | キャップストーン: 1 つの熱拡散を ODE/PDE/Fourier の 3 視点で(全て一致) |
...
済: 全章演習解答(`08`)、発展章(`09`: スペクトル法・KdV ソリトン・反応拡散・2D 波動)、
キャップストーン(`10`: 1 現象を ODE/PDE/Fourier の 3 視点で串刺し)、
Plotly インタラクティブ(Fourier 項数・熱 `r`・CFL `C`・Black-Scholes 3D 曲面)、
**Crank-Nicolson**・**Neumann(保存形)**・**非線形 Burgers** ソルバ、Poisson のベクトル化、
