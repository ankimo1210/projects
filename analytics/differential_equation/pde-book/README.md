...
(`calculus.py` / `grids.py` / `solvers.py` / `plotting.py` / `widgets.py` / `datasets.py`)。
...
- Neumann/Robin 境界条件と非長方形領域、有限要素法 (FEM) の入口
- 2 次元の波動・移流、非線形 PDE(Burgers・反応拡散・KdV)
- スペクトル法(FFT ベース)と Crank-Nicolson 法の実装比較
