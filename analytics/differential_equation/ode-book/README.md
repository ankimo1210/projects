...
(`calculus.py` / `solvers.py` / `systems.py` / `plotting.py` / `widgets.py` / `datasets.py`)。
...
- 境界値問題(BVP)とシューティング法 / 有限要素の入口
- 確率微分方程式 (SDE) の本格導入(07 章 Vasicek の発展)
- Neural ODE の実学習デモ(現状は概念図のみ)
- アニメーション(`plotting.line_animation`)を使った軌道・位相の動的可視化を各章に常設
