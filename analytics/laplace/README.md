...
2026-08-17 に 03・07・08・11 の「今後の拡張(TODO)」をすべて決着させた。00 は本書の地図として短いままで、
それ以外の全章が実装済み。

| 旧 TODO | 決着 |
|---|---|
| 03: ヘヴィサイド展開定理の手計算 / `scipy.signal.residue` / むだ時間 $e^{-as}$ | §10b・§10c に実装。むだ時間では数値逆変換が段差をなますこと(誤差 0.55)も測っている |
| 07: フィルタ付き微分の PID / 状態空間表現 / 離散化 | §6d〜§6f に実装。$N$ を 5→500 にするとノイズ由来のピークが 3.1→9.1 に増える |
| 08: LST と M/M/1 / 期間構造 / 特性関数 / SDE 生成作用素 | §4b〜§4d に実装(M/M/1 は Lindley 再帰 20 万サンプルで LST を実測)。**SDE の生成作用素だけは範囲外**とし、`differential_equation/sde-book` に譲った |
| 11: サンプリング定理 / DTFT・DFT / 双一次変換 / コントローラ離散化 | §7b〜§7d に実装 |
...
- 03 章: ヘヴィサイド展開定理の手計算、むだ時間 $e^{-as}$ の逆変換(数値部分分数は実装済み)
