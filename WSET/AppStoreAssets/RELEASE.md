...
- 2026-09-16の確認では、1.0は承認済みだが配信対象から削除されていた。配信対象地域が未設定だったため、日本のみを再設定し「配信可能を処理中」を確認。解除した操作主体・日時は未特定。ストア反映は最大24時間。
- 2026-09-16 13:43 JST、ビルド2を1.0.1へ紐づけて審査へ提出。APIで`WAITING_FOR_REVIEW`と`AFTER_APPROVAL`を確認。
- Sandbox購入・復元確認とストア製品ページ確認は引き続き未完了。未確認事項を説明した後のユーザーによる配信再指示に従い、配信復旧・審査提出を実行した。チェックリストを検証済みには変更しておらず、自動ReleaseゲートのNO-GOは残る。
