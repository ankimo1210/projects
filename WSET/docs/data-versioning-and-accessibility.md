...
同じIDの理論模試では、開始日時／提出日時だけで回答の更新順序は判別できない。両方が回答中の場合は端末の回答を優先し、未登録の項目だけバックアップから補完する（明示的な空回答も保持）。両方が自己採点中の場合は、提出済み答案と採点の整合性を保つため端末の答案・採点状態を保持する。端末の表示位置とフラグは維持する。完了済み模試は未完了状態へ戻さず、同じ完了日時の復元でも上書きしない。
