# CruNote for WSET 全体開発計画

作成日: 2026-07-18

対象: 日本語・オフライン WSET Level 3 iOSアプリ

状態: R0〜R7主要技術実装・自動検証済み／Swift単体115/115・UI 26/26・Python 59/59／商用ReleaseはNo-Go
...
| [WSET Level 3 Exam](https://apps.apple.com/jp/app/wset-level-3-exam/id1556321093) | アプリ¥2,500 | 英語、オフライン、カード・弱点練習 | 日本語・記述式・本番模試を含む複合学習体験なら、高価格帯を検証する余地がある |
...
- 発売時¥1,500、通常¥5,000を検証開始点とする。2026年8月17日に通常価格へ移行し、直接競合の¥480へ追随せず、広告なし、記述式、120分模試、説明可能な弱点推薦、地図比較、SATを価格理由にする。
- 編集レビューは販売可否の必須条件にしない。レビュー情報は品質改善の履歴として保持し、構造検証・単体/UIテスト・Release buildで配布品質を担保する。
...
実装完了と商用公開可否は別々に判定する。問題の編集レビュー状態はRelease配布の条件にせず、App Store作業と実機QAを別途完了させる。

| トラック | 現在地 | 次の作業 | Exit gate |
|---|---|---|---|
| アプリ実装 R0〜R7 | Swift単体115/115、UI 26/26、Python 59/59、Release build成功 | 変更を固定し、コンテンツ・実機QA・ストアの外部ゲートを閉じる | 達成（全単体・UI・生成物・Release build成功） |
| 四択・記述コンテンツ | 四択1100問、記述10問をRelease配布 | 利用者フィードバックと任意レビューで継続改善 | 全1110問がReleaseパックへ収録される |
...
日常検証は`make verify`、App Store archive直前の厳格な判定は`make release-check`を使用する。後者は問題パックの件数・Release状態、地図出典、手動チェックリスト、プライバシーポリシーのいずれかが未完了なら非0で終了する。

問題レビューをRelease条件から外した後も、`make verify`、Swift単体、UI、Release buildで構造・動作を検証する。商用ゲートはApp Store手動項目、プライバシーポリシー、素材の出典・利用条件を中心に判定する。
...
1. 完了（2026-07-19）: R0〜R7の受入条件監査を閉じ、全単体・UI・生成物・Release buildを再実行する。
2. 四択1100問と記述10問をReleaseパックへ生成し、`make verify`で整合性を確認する。
3. 地図の出典、商標・利用条件とWSET非提携表記を確認する。
4. App Store Connectへ`pro_lifetime`を登録し、価格、説明、スクリーンショット、サポートURL、App Privacyを確定する。
5. StoreKit Sandboxで購入、キャンセル、保留、返金・取消、復元、再インストール、オフライン権利を確認する。
6. TestFlight実機でVoiceOver、最大Dynamic Type、ダークモード、機内モード、バックグラウンド復帰、旧バックアップ復元を確認する。
7. プライバシーポリシーを運営者情報・保持期間込みで公開版へ確定し、手動チェックリストを証跡付きで完了する。
8. `make release-check`が`GO`になった同一revisionからArchiveを作成する。R7は別の有効化条件を満たすまでOFFを維持する。
