...
最終監査スナップショットは次のとおり。`make verify`はPython 59/59と全生成物検査が成功、Swift単体は115/115、UIは26/26が成功した。R5語彙検索は、フォームの上下安全領域を考慮したスクロール、検索キーによるキーボード終了、候補行全体のヒット領域化、追加通知状態の親ビュー保持により解消した。Release buildも成功した。商用ゲートはコンテンツ5件、手動確認12件、プライバシー1件の合計18件を検出して正しくNo-Goとなる。
