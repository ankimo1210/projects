...
CruNoteは、日本語で学習できる個人利用向けのオフライン WSET 学習アプリです。現在はLevel 3に対応し、問題データと学習履歴は端末内に保存されます。

## 主な機能

- 自作した日本語4択1100問
- 採点基準付き記述式10問、入力下書き、過去回答・得点推移、不足用語復習
...
四択1100問はすべて`distributionStatus: release`の生成物へ収録します。`reviewStatus`と`needsReview`は編集履歴と改善候補を追跡するメタデータとして残しますが、Release配布の条件には使用しません。

任意の外部レビューは[`docs/content-review-workflow.md`](docs/content-review-workflow.md)に従い、依頼対象ハッシュ、指摘、修正、正本の承認状態を追跡できます。次のコマンドで現在の四択1100問、記述式10問、地図・比較素材に結び付いた依頼パケットを生成します。

```sh
python3 scripts/build_content_review_packet.py
```

レビューを実施する場合は、担当者・確認日とレビュー時点の内容ハッシュを保存します。未解決指摘は改善管理に使いますが、商用Releaseゲートはブロックしません。

記述式問題の正本は `QuestionSources/wset_level3_written_questions.json` です。

Release配布用は、`rejected`以外の記述式10問をすべて生成します。

```sh
python3 scripts/build_written_question_pack.py
```

生成先は `WSET/QuestionData/written_question_pack.json` です。配点と採点基準の合計、関連用語ID、メタデータ、参照パック依存ハッシュ、決定的出力を生成時に検証します。レビュー状態は任意の編集履歴として保持します。
...
これは四択・記述式パックの件数とRelease状態、地図出典、App Store手動チェックリスト、プライバシーポリシー確定を検査します。専門家レビューの有無はRelease判定に使いません。
...
python3 scripts/build_written_question_pack.py --check
