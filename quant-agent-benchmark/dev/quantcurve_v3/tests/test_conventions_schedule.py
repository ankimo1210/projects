...
def test_accruals_legacy_level_and_contract_actual():
    t = schedule_times(1.5, 1, "forward")
    np.testing.assert_allclose(ois_accruals(t, 1, "forward"), [1.0, 1.0])  # legacy reading: level 1/f
    np.testing.assert_allclose(ois_accruals(schedule_times(2.5, 2), 2), np.full(5, 0.5))  # on-grid: contract == level
    np.testing.assert_allclose(ois_accruals(schedule_times(1.5, 1), 1), [1.0, 0.5])  # contract v2: actual stub accrual
    for rule in ("contract_v2",):
        for T in (0.3, 1.25, 1.5, 4.066, 13.73, 29.78):
            for f in (1, 2, 4):
                t = schedule_times(T, f, rule)
                assert t[-1] == pytest.approx(T) and np.all(np.diff(t) > 0) and t[0] > 0
                assert np.all(t[:-1] < T) and np.allclose(t[:-1] * f, np.round(t[:-1] * f))
