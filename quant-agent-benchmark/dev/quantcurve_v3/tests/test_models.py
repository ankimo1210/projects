...
def test_make_knots_adds_bond_maturities_only_where_rates_are_sparse():
    """dev v3: bond maturities become knot candidates only farther than bond_gap/2 from every rate tenor."""
    import numpy as np
    from quantcurve.advanced import make_knots

    mats = np.array([0.5, 1.0, 2.0, 3.0, 5.0, 8.0, 12.0, 12.3, 14.0, 16.0, 19.0, 25.0])
    types = np.array(["deposit", "ois_swap", "ois_swap", "ois_swap", "ois_swap", "ois_swap", "ois_swap", "bond", "bond", "bond", "bond", "bond"])
    base = make_knots(mats, types, 30.0, 40)
    assert not np.any(np.isin([14.0, 16.0, 19.0, 25.0], base))  # rates only by default
    with_bonds = make_knots(mats, types, 30.0, 40, bond_gap=1.5)
    assert np.all(np.isin([14.0, 16.0, 19.0, 25.0], with_bonds))  # bonds far from every rate tenor are added
    assert 12.3 not in with_bonds  # a bond 0.3Y from the 12Y OIS tenor is not (within bond_gap / 2)



def test_cv_loss_variants_run_and_factor_square_matches_square_when_factors_are_one():
    """dev v3: the CV loss options produce a valid lambda; with unit factors the factor-weighted loss equals the plain one."""
    import numpy as np
    from datetime import date
    ...
    df = synthetic_frame(0.02, 0.5, 5)
    with tempfile.TemporaryDirectory() as tmp:
        p = pathlib.Path(tmp) / "m.csv"; df.to_csv(p, index=False)
        lams = {}
        for loss in ("square", "factor_square", "huber3"):
            res = run_workflow(WorkflowOptions(market_data=p, output_dir=pathlib.Path(tmp) / loss, valuation_date=date(2026, 1, 15), skip_sensitivity=True, cv_loss=loss))
            assert np.isfinite(res.adv.lam) and res.adv.lam > 0
            lams[loss] = (res.adv.lam, res.adv.cv.table["cv_score"].to_numpy())
        if (res.adv.fit.robust_factor == 1.0).all():
            np.testing.assert_allclose(lams["square"][1], lams["factor_square"][1], rtol=1e-9)
