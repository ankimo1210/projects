...
def synthetic_frame(level: float = 0.02, noise_bp: float = 0.0, seed: int = 1, stub_rule: str = "contract_v2") -> pd.DataFrame:
    ...
    for T in (1, 1.25, 1.5, 2, 2.5, 3, 4, 5, 6, 7, 8, 9, 10, 12, 15, 20, 25, 30):
        ...
        alphas = np.diff(np.concatenate([[0.0], times]))
        par = (1 - true_discount(T, level)) / np.sum(alphas * true_discount(times, level)) * 100
        for _ in range(2):
    ...
    for T in np.linspace(1.6, 29.5, 20):
        ...
        alphas = np.diff(np.concatenate([[0.0], times]))
        price = np.sum(100 * c * alphas * true_discount(times, level)) + 100 * true_discount(T, level)
