...
def build_instrument(
    ...
    if frequency <= 0:
        raise ValueError(f"{instrument_id}: payment_frequency must be positive")
    times = schedule_times(maturity, frequency, stub_rule)
    # contract v2: coupon_i = 100 * c * alpha_i (stub proportional to accrual);
    # legacy level rules have alpha_i = 1/m so this reduces to 100 c / m.
    alphas = ois_accruals(times, frequency, stub_rule)
    amounts = 100.0 * float(coupon_rate) * alphas
    return Instrument(instrument_id, instrument_type, float(maturity), float(quote), times, alphas, amounts, frequency, float(coupon_rate))
