...
def clean_market_data(frame: pd.DataFrame, valuation_date: date, config: CleaningConfig | None = None) -> CleaningResult:
    ...
    for i in range(n):
        ...
        if itype[i] in ("ois_swap", "bond"):
            ...
            elif itype[i] == "ois_swap":
                rule_freq = 1.0 if mat[i] <= 2.0 else 2.0
                if int(freq[i]) != int(rule_freq):
                    states[i].flag(f"payment_frequency {int(freq[i])} disagrees with the OIS convention (annual T<=2, semiannual T>2); {int(rule_freq)} applied", action="correct")
                    freq[i] = rule_freq
        else:
            freq[i] = 1.0
