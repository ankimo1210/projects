...
def render_report(res, out_path: Path) -> Path:
    ...
    stub_note = ""
    for c in sens_checks:
        if c["name"] == "stub_rule_forward":
            stub_note = f"Under the inherited legacy reading (<code>forward</code>: n = round(T f) level payments) the in-sample RMSE over usable instruments rises to {_fmt(c['outcome']['train_rmse_bp'].get('all'))}bp (vs {_fmt(ts['advanced']['usable']['overall']['rmse_bp'])}bp under the contract) and the zero curve moves by up to {_fmt(c['outcome']['max_abs_zero_change_bp'])}bp."
    ...
    if stub_note:
        findings.append("Cash-flow schedules follow the canonical pricing contract v2.0 (coupon dates at every k/m strictly before T, then T; actual accruals; stub coupon proportional to accrual; principal at T), verified against an independent enumeration and QuantLib discount factors. " + stub_note)
    if consensus:
    ...
    parts.append(
        ...
        "bonds pay coupons <code>100 c &alpha;<sub>i</sub></code> at their payment dates with principal at maturity and no accrued interest; <code>settlement_days</code> is metadata only.</li>"
        f"<li>Schedule rule <code>{o.stub_rule}</code>: coupon dates at every <code>k/m</code> strictly before <code>T</code> and then <code>T</code> itself (final short stub, no rounding of <code>T &times; m</code>), accruals <code>&alpha;<sub>i</sub> = t<sub>i</sub> - t<sub>i-1</sub></code> with <code>t<sub>0</sub> = 0</code>; OIS annual to 2Y and semi-annual beyond (a supplied frequency that disagrees is corrected and flagged). "
        "Verified in <code>tests/test_contract_v2.py</code> against an explicit enumeration of the contract text and QuantLib 1.43 discount factors on continuous year fractions (T shorter than one period, on/off grid, zero coupon, negative rates with D &gt; 1). The legacy readings of earlier rounds are kept only as sensitivity alternatives (section 5).</li>"
