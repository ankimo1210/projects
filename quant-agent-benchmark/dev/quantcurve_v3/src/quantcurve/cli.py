...
def build_parser() -> argparse.ArgumentParser:
    ...
    run.add_argument("--penalty-shape", choices=["power", "waggoner_steps"], default="power", help="roughness-penalty maturity weighting")
    run.add_argument("--max-knots", type=int, default=40, help="cap on interior forward-spline knots (28 = rounds 1-2 setting)")
    run.add_argument("--no-loo-screen", action="store_true", help="disable the leave-tenor-out outlier screen")
    run.add_argument("--knot-bond-gap", type=float, default=1.5, help="bond maturities become knots where rate tenors are sparser than this many years (default 1.5; <=0 disables)")
    run.add_argument("--stub-rule", choices=["contract_v2", "forward", "round", "linspace", "ceil", "forward_actual"], default=DEFAULT_STUB_RULE, help="schedule rule for non-integer tenors")
    ...
    run.add_argument("--grid-step", type=float, default=1.0 / 48.0, help="grid spacing in years (default 1/48; the grid starts at one step and always contains 1/12Y)")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        opts = WorkflowOptions(
            ...
            penalty_shape=args.penalty_shape,
            max_knots=args.max_knots,
            use_loo_screen=not args.no_loo_screen,
            knot_bond_gap=(args.knot_bond_gap if args.knot_bond_gap and args.knot_bond_gap > 0 else None),
