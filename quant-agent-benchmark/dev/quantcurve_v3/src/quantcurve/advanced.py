...
class AdvancedConfig:
    ...
    max_knots: int = 40  # round 02: 28 -> 40 (H3, see audit/experiments); the penalty controls smoothness
    penalty_shape: str = "power"  # "power" (Anderson-Sleath style) or "waggoner_steps" (Waggoner 1997 ratios)
    use_loo_screen: bool = True
    knot_bond_gap: float | None = 1.5  # bond maturities become knots where rate tenors are sparser than this (years); None = rates only
    cv_loss: str = "square"  # "square" | "factor_square" (held-out u^2 weighted by the robust factor) | "huber3" (factor-weighted Huber rho, c=3)
    lambda_grid_points: int = 22  # points of the log-spaced lambda grid on [1e-3, 1e4]
    loo_window: int = 0  # neighbouring tenor clusters (each side) left out together with the tested cluster (anti-masking)
...
def make_knots(maturities: np.ndarray, types: np.ndarray, t_max: float, max_knots: int = 28, bond_gap: float | None = None) -> np.ndarray:
    """Interior knots at the tenor clusters of rate instruments (fallback: all).

    ``bond_gap``: if given, bond maturities that lie farther than ``bond_gap / 2``
    from every rate tenor are added as knot candidates, so regions covered only
    by bonds (e.g. a sparse long end) still get local flexibility.
    """
    ...
    if bond_gap is not None and rate_mask.sum() >= 4 and (~rate_mask).any():
        bonds = np.unique(np.round(maturities[~rate_mask], 6))
        far = np.array([np.min(np.abs(cand - m)) > bond_gap / 2.0 for m in bonds]) if len(cand) else np.ones(len(bonds), bool)
        cand = np.unique(np.concatenate([cand, bonds[far]]))
...
def penalty_weight(cfg: "AdvancedConfig", power: float | None = None):
    ...
    if power == 0:
        return None
    if cfg.penalty_shape == "waggoner_steps":
        # Waggoner (1997): lambda(t) = 0.1 (t < 1), 100 (1 <= t < 10), 100000 (t >= 10); only the ratios matter
        # because the overall level is re-selected by cross-validation.
        def weight_steps(t):
            t = np.asarray(t, dtype=float)
            return np.where(t < 1.0, 1e-3, np.where(t < 10.0, 1.0, 1e3))
...
def leave_tenor_out_screen(
    ...
    mats_c = np.array([i.maturity for i in insts])
    order = sorted(np.unique(cluster_ids), key=lambda c: float(np.median(mats_c[cluster_ids == c])))
    rank = {c: k for k, c in enumerate(order)}
    for cid in np.unique(cluster_ids):
        test = cluster_ids == cid
        if cfg.loo_window > 0:
            # leave the neighbouring clusters out as well so that two adjacent
            # bad quotes cannot mask each other
            r = rank[cid]
            held = np.isin(cluster_ids, order[max(0, r - cfg.loo_window): r + cfg.loo_window + 1])
        else:
            held = test
        train = ~held
...
def cross_validate_lambda(
    ...
    grid = cfg.lambda_grid if cfg.lambda_grid_points == 22 else np.logspace(-3, 4, cfg.lambda_grid_points)

    def loss(u: np.ndarray, w: np.ndarray) -> tuple[float, float]:
        """Weighted robust loss of standardised held-out residuals: (sum of w*rho(u), sum of w)."""
        if cfg.cv_loss == "square":
            return float(np.sum(u**2)), float(len(u))
        rho = u**2 if cfg.cv_loss == "factor_square" else np.where(np.abs(u) <= 3.0, u**2, 3.0 * (2.0 * np.abs(u) - 3.0))
        return float(np.sum(w * rho)), float(np.sum(w))

    for lam in grid:
        sq, count, per_fold = 0.0, 0.0, []
        for f in fold_ids:
            train = folds != f
            ...
            l_sum, l_w = loss(u, fixed_factor[test])
            per_fold.append(float(np.sqrt(l_sum / max(l_w, 1e-12))))
            sq += l_sum
            count += l_w
        rows.append({"lambda": float(lam), "cv_score": float(np.sqrt(sq / max(count, 1e-12))), "fold_scores": per_fold})
...
def fit_advanced(
    ...
    cluster_ids = np.asarray(cluster_ids)
    if knots is None:
        knots = make_knots(maturities, types, t_max, cfg.max_knots, cfg.knot_bond_gap)
    ...
    fit = robust_fit(engine, base_scale, types, lam, L, theta0, cfg, cluster_ids=cluster_ids, run_loo=cfg.use_loo_screen)
