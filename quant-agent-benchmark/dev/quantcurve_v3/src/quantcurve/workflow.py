...
class WorkflowOptions:
    ...
    seed: int = 20260115
    penalty_shape: str = "power"
    max_knots: int = 40
    use_loo_screen: bool = True
    knot_bond_gap: float | None = 1.5
    cv_loss: str = "square"
    lambda_grid_points: int = 22
...
def _run(opts: WorkflowOptions, timings: dict) -> WorkflowResult:
    ...
    cfg = AdvancedConfig(n_folds=opts.n_folds, lambda_fixed=opts.lambda_fixed, penalty_shape=opts.penalty_shape, max_knots=opts.max_knots, use_loo_screen=opts.use_loo_screen, knot_bond_gap=opts.knot_bond_gap, cv_loss=opts.cv_loss, lambda_grid_points=opts.lambda_grid_points, cv_max_tolerance=opts.cv_max_tolerance, loo_window=opts.loo_window)
    ...
    grid = np.arange(1, n_steps + 1) * opts.grid_step  # starts at one step (below 1/12Y when the step is finer)
    if not np.any(np.abs(grid - 1.0 / 12.0) < 1e-9):
        grid = np.sort(np.append(grid, 1.0 / 12.0))  # the contract's 1/12Y point is always present
