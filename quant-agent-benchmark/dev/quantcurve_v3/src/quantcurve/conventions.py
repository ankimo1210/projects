...
StubRule = Literal["contract_v2", "round", "forward", "ceil", "linspace", "forward_actual"]
...
DEFAULT_STUB_RULE: StubRule = "contract_v2"

#: Round-02 canonical pricing contract v2.0 (``CONVENTIONS.md``): coupon dates at
#: every ``k/m`` strictly before ``T`` and then ``T`` itself (final short stub),
#: accruals ``alpha_i = t_i - t_{i-1}`` with ``t_0 = 0``; stub coupons are
#: proportional to their accrual. This supersedes the legacy readings below,
#: which are kept only as sensitivity alternatives.
CONTRACT_V2_RULE: StubRule = "contract_v2"
...
def schedule_times(maturity_years: float, frequency: int, rule: StubRule = DEFAULT_STUB_RULE) -> np.ndarray:
    ...
    * ``"forward"``: ``n = max(1, round(T f))`` payments at ``1/f, 2/f, ...``
    ...
    if rule == "contract_v2":
        # every k/m strictly before T, then T itself (final short stub)
        k_max = int(math.floor(periods - 1e-9))
        regular = np.arange(1, k_max + 1) / frequency
        regular = regular[regular < maturity_years - 1e-12]
        times = np.concatenate([regular, [maturity_years]])
    elif rule == "round":
...
def ois_accruals(times: np.ndarray, frequency: int, rule: StubRule = DEFAULT_STUB_RULE) -> np.ndarray:
    ...
    """
    if rule in ("contract_v2", "ceil", "forward_actual"):
