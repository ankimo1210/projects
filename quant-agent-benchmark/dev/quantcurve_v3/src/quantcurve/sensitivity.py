...
def _interpret(check: dict) -> str:
    """Plain-language reading of one check's numbers (relative to quote-noise scale)."""
    oc = check["outcome"]
    name = check["name"]
    if "max_abs_deviation_from_1bp" in oc:
        return (f"A +1bp parallel shift of every quote moves zero rates by {oc['mean_zero_change_bp']:.3f}bp on average "
                f"(range {oc['min_zero_change_bp']:.3f} to {oc['max_zero_change_bp']:.3f}bp); the maximum deviation from the expected 1bp is "
                f"{oc['max_abs_deviation_from_1bp']:.3f}bp, so the estimator passes level shifts through linearly.")
    if "max_zero_std_bp" in oc:
        return (f"Across {oc['n_replications']} bid/ask-scaled noise replications the zero-rate standard deviation is at most {oc['max_zero_std_bp']:.2f}bp "
                f"(median {oc['median_zero_std_bp']:.2f}bp): quote noise of the observed size does not destabilise the curve.")
    if "most_influential" in oc:
        top = oc["most_influential"][0] if oc["most_influential"] else None
        where = f"the {top['maturity_years']:.2f}y cluster" if top else "no cluster"
        return (f"Deleting one tenor cluster at a time changes the zero curve by at most {oc['max_abs_zero_change_bp']:.2f}bp; the most influential "
                f"cluster is {where}. Influence is concentrated where the curve is least covered.")
    mx = oc.get("max_abs_zero_change_bp")
    if mx is None:
        return "No curve-level comparison available for this check."
    level = "negligible (below typical quote noise)" if mx < 1.0 else ("moderate (within the bid/ask scale)" if mx < 3.0 else "material - this factor moves the curve beyond quote noise and is listed under Limitations")
    txt = f"Maximum zero-rate change versus the selected curve is {mx:.2f}bp (mean {oc.get('mean_abs_zero_change_bp', float('nan')):.2f}bp, forward up to {oc.get('max_abs_forward_change_bp', float('nan')):.2f}bp): {level}."
    tr = oc.get("train_rmse_bp", {}).get("all") if isinstance(oc.get("train_rmse_bp"), dict) else None
    if tr is not None:
        txt += f" In-sample RMSE over usable instruments under this condition: {tr:.2f}bp."
    if name.startswith("stub_rule_"):
        txt += (" The public conventions do not pin the payment dates or stub accrual of non-integer tenors; this reading is reported as an "
                "alternative interpretation, not as an error of the data.")
    return txt
...
def run_sensitivity(
    ...
    for rule in ("forward", "forward_actual"):  # legacy readings, for the size of the convention effect only
        if rule == stub_rule:
            continue
        alt_insts = [
    ...
    return {c["name"]: {"condition": c["description"], "results": c["outcome"], "interpretation": _interpret(c)} for c in checks}, pd.DataFrame(deltas)
