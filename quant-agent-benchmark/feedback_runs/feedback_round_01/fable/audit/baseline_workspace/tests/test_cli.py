...
def test_end_to_end_outputs(dataset, tmp_path):
    ...
    assert len(sens["checks"]) >= 3
    assert all("name" in c and "outcome" in c for c in sens["checks"])
    ...
    for section in ("Executive summary", "Methodology", "Data-quality findings", "Baseline versus advanced", "Sensitivity analysis", "Validation and repricing", "Charts", "Limitations", "Recommended next steps"):
