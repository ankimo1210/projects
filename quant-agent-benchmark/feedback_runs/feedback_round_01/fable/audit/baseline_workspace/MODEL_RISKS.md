...
2. **Schedule rule for non-integer tenors (undocumented, chosen empirically).**
   `n = round(T·f)` payments at `1/f, 2/f, …` from the valuation date with the
   final payment moved to maturity (`--stub-rule forward`). Evidence: the
   textbook short-stub rule (`ceil`) leaves the 1.25Y and 1.5Y OIS quotes
   ~50bp inconsistent with 1Y/2Y and adds a full spurious coupon to bonds
   with a short front stub (in-sample RMSE 21bp vs 2.5bp); among the
   round-based variants (`forward`, `round`, `linspace`) the curve moves by
   at most 0.25bp, so the residual convention risk is small unless the
   counterparty uses a short-stub convention.
