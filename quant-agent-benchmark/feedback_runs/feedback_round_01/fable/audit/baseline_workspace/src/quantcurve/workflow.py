...
def _run(opts: WorkflowOptions, timings: dict) -> WorkflowResult:
    ...
    model_comparison = {
        ...
        "baseline": {"description": "Sequential bootstrap over tenor-cluster quotes of deposits and OIS with linear interpolation of continuously compounded zero rates and flat zero extrapolation; bonds unused.", "train": train_metrics["baseline"], "holdout": holdout.metrics["baseline"]},
        "advanced": {
    ...
    if opts.skip_sensitivity:
        sensitivity, deltas = {"checks": [], "skipped": True}, None
    else:
