...
Tests cover positive discount factors, negative rates, zero/discount/forward consistency, continuity, numerical stability, deterministic fitting, auditable missing data and duplicates, unit conversion, bid/ask inversions, robust outliers, direct-`D(T)` pricing including fractional maturities, DV01 finite differences, key-rate aggregation, CLI execution, output schemas, and end-to-end byte reproducibility.
...
- model comparison with product/tenor/schedule slices, structured sensitivity experiments, validation, and run metadata JSON;
- four PNG charts; and
...
`quantcurve.pricing.model_quote_from_discount` accepts a known discount function directly. It is intended for independent convention and unit tests that do not pass through curve interpolation. The production `model_quote` delegates to the same pricing path.
