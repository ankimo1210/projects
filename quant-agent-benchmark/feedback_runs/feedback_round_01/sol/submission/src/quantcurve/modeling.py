...
def fit_metrics(rows: pd.DataFrame, curve: ZeroCurve, config: CurveConfig | None = None) -> dict[str, object]:
    ...
    result: dict[str, object] = {
        ...
        "weighted_normalized_rmse_unit": "dimensionless_spread_units",
        "normalized_mae": float(np.mean(np.abs(standardized))),
        "normalized_mae_unit": "dimensionless_spread_units",
    }
...
def segmented_fit_metrics(rows: pd.DataFrame, curve: ZeroCurve, config: CurveConfig | None = None) -> dict[str, object]:
    """Return non-pooled diagnostics for the fixed tenor and schedule slices."""
    cfg = config or CurveConfig()
    maturities = rows["maturity_years"].to_numpy(dtype=float)
    bands = {
        "short_T_le_2": maturities <= 2.0,
        "medium_2_lt_T_lt_15": (maturities > 2.0) & (maturities < 15.0),
        "long_T_ge_15": maturities >= 15.0,
    }
    result: dict[str, object] = {"by_tenor": {}, "by_maturity_convention": {}}
    for name, mask in bands.items():
        result["by_tenor"][name] = None if not np.any(mask) else fit_metrics(rows.loc[mask].reset_index(drop=True), curve, cfg)
    fractional = np.abs(2.0 * maturities - np.round(2.0 * maturities)) > 1e-8
    for name, mask in {"regular_half_year_grid": ~fractional, "fractional_maturity": fractional}.items():
        result["by_maturity_convention"][name] = None if not np.any(mask) else fit_metrics(rows.loc[mask].reset_index(drop=True), curve, cfg)
    return result
