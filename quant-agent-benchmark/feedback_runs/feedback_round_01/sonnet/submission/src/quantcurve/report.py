...
def render_report(ctx: dict, chart_files: dict[str, Path], out_path: Path) -> None:
    ...
    fwd_smooth = sens.get("forward_smoothness_check")
...
{"" if fwd_smooth is None else f'''<div class="panel">
  <h3>Forward-rate smoothness at calibration knots</h3>
  <p>Zero-rate repricing error and forward-rate error are different diagnostics: f(t)=z(t)+t&middot;z&prime;(t) amplifies
  any slope discontinuity by t. The baseline&#39;s piecewise-linear zero rate is kinked at every knot by construction;
  the advanced spline is smooth (C1). Worst internal-knot forward jump: <strong>{_fmt(fwd_smooth["baseline_max_jump_bp"],1)} bp</strong>
  for baseline (at t={_fmt(fwd_smooth["worst_baseline_knot"]["knot"],3)}y, mean {_fmt(fwd_smooth["baseline_mean_jump_bp"],1)} bp
  across all internal knots) vs. <strong>{_fmt(fwd_smooth["advanced_max_jump_bp"],3)} bp</strong> for advanced (mean
  {_fmt(fwd_smooth["advanced_mean_jump_bp"],3)} bp). A small holdout-RMSE gap between the two models can therefore still
  hide a large difference in forward-curve quality; see <code>MODEL_RISKS.md</code> for the full discussion.</p>
</div>'''}
