...
210 tests. Expect **about 115 seconds**: roughly 80 of them are one end-to-end
test that runs the mandated CLI on the real 143-row data set in a subprocess.
That test finds the data set via `QUANTCURVE_MARKET_DATA` or by searching
ancestor directories for `input/market_data/market_observations.csv`, and skips
itself with a message only if neither resolves &mdash; no path is hardcoded; everything else runs on synthetic data generated from a known
...
tests/            210 tests; synthetic.py generates data from a known curve
