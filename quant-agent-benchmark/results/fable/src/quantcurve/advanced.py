...
def type_scales(std_res: np.ndarray, types: np.ndarray, floor: float, min_members: int) -> dict[str, float]:
...
def leave_tenor_out_screen(
    ...
    scales = type_scales(loo[factor > 0], types[factor > 0], cfg.scale_floor, cfg.min_type_members)
...
def robust_fit(
    ...
    if fixed_factor is not None:
        ...
        scales = type_scales(r[factor > 0], types[factor > 0], cfg.scale_floor, cfg.min_type_members)
        s_vec = np.array([scales[str(t)] for t in types])
    ...
    for _ in range(cfg.max_iterations):
        r = engine.residuals(theta / PCT) / base_scale
        good = factor > 0
        scales = type_scales(r[good], types[good], cfg.scale_floor, cfg.min_type_members)
        s_vec = np.array([scales[str(t)] for t in types])
        u = r / s_vec
