...
def render_report(res, out_path: Path) -> Path:
    ...
    sens_checks = res.sensitivity.get("checks", [])
    ...
    parts = [f"<title>QuantCurve research report {html.escape(str(o.valuation_date))}</title><style>{CSS}</style>"]
    parts.append(
    ...
    # --- executive summary ---------------------------------------------
    parts.append("<section id='summary'><h2>1. Executive summary</h2>")
    parts.append(
        "<div class='kpi'>"
    ...
    if stub_note:
        findings.append("Cash-flow schedules for tenors that are not a whole number of periods are not defined by the conventions document; the rule was chosen empirically. " + stub_note)
    if consensus:
    ...
    # --- methodology --------------------------------------------------
    parts.append("<section id='method'><h2>2. Methodology</h2>")
    parts.append(
    ...
    parts.append(
        ...
        "(ii) iteratively reweighted least squares with Tukey's biweight (c = 4.685); (iii) a consensus guard that never rejects a whole tenor cluster whose quotes agree with each other (their weight is capped by Huber weights instead). "
    ...
    # --- data quality -------------------------------------------------
    parts.append("<section id='data'><h2>3. Data-quality findings</h2>")
    ...
    # --- model comparison ---------------------------------------------
    parts.append("<section id='models'><h2>4. Baseline versus advanced model</h2>")
    ...
    # --- sensitivity ----------------------------------------------------
    parts.append("<section id='sens'><h2>5. Sensitivity analysis</h2>")
    if sens_checks:
        parts.append(_img(res.files.get("chart_sensitivity")))
        parts.append(_table(sens_rows(), "{:.2f}"))
        parts.append("<ul>" + "".join(f"<li><b>{html.escape(c['name'])}</b>: {html.escape(c['description'])}</li>" for c in sens_checks) + "</ul>")
    ...
    parts.append("<section id='valid'><h2>6. Validation and repricing</h2>")
    ...
    # --- risk -----------------------------------------------------------
    parts.append("<section id='risk'><h2>7. Risk: DV01 and key-rate sensitivities</h2>")
    ...
    # --- charts ---------------------------------------------------------
    parts.append("<section id='charts'><h2>8. Charts</h2><h3>Zero curves</h3>" + _img(res.files.get("chart_curve")) + "<h3>Forward rates</h3>" + _img(res.files.get("chart_forward")) + "</section>")
    ...
    parts.append("<section id='limits'><h2>9. Limitations and model risk</h2><ul>" + "".join(f"<li>{x}</li>" for x in lim) + "</ul><p class='small'>See <code>MODEL_RISKS.md</code> for the full model-risk discussion.</p></section>")
    nxt = [
    ...
    parts.append("<section id='next'><h2>10. Recommended next steps</h2><ol>" + "".join(f"<li>{x}</li>" for x in nxt) + "</ol></section></main>")
