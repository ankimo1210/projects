...
> 無キーソースは**ライブ実証済**(SEC EDGAR / yfinance / Binance / US Treasury、`IRP_LIVE=1`)。
...
```bash
uv run pytest investment-research-platform/tests -q          # 128 passed, 7 skipped(ネット不要)
IRP_LIVE=1 uv run pytest investment-research-platform/tests/test_live.py -q   # 実 API スモーク
```
...
品質診断が欠損/ギャップ/重複を検出、コネクタ正規化。**ライブテスト**(`IRP_LIVE=1`、既定スキップ)は
...
- **FX/通貨換算**: `irp.data.to_base_currency` で基軸通貨へ換算(前埋め無し)。取引所カレンダー差は
...
- **Phase 2c(完了)** — 広さの拡充:全6信号ファミリ・モデル Tier3-4・配当課税、
  **全データコネクタ**(J-Quants / e-Stat / SEC EDGAR / BoJ / MoF / BLS / BEA / Census / EDINET、
  ...
  monkeypatch で配線検証)、**ライブ実証**(無キーソースを実 API で、`IRP_LIVE=1`)。
