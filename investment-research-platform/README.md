...
> **状態: Phase 2a — リサーチ層**。データ層(Phase 1)に加え、**causal な特徴量**・
> **共通スキーマの信号(trend/risk/macro ベースライン)**・**forward ラベル(embargo 用の
> 公表日付き)**まで実装。モデル・walk-forward バックテスト・税・ポートフォリオ・
> ダッシュボードは次フェーズ([下記ロードマップ](#ロードマップ))。
...
investment-research-platform/
  ...
  notebooks/ 01..13     (01-05 実走、06-13 は次フェーズのスタブ)
  src/irp/   data/ macro/ features/ signals/ labels/ utils/   ← 実装済み
             models/ backtest/ portfolio/ tax/ visualization/  ← スタブ
  tests/
```
...
- **Phase 2b(次)** — モデル(Tier0 ベースライン〜Tier4 時系列基盤モデル)→ **walk-forward
  バックテスト**(コスト/スリッページ/税ドラッグ/ターンオーバー/ベンチ/ベースライン比較・
  ラベル embargo)→ 日本税(NISA)→ ポートフォリオ構築 → Plotly ダッシュボード →
  NB 06–13 実走 → 最終 HTML レポート(失敗戦略も提示)。
