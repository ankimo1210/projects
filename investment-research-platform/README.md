...
> **状態: Phase 2b-2 — モデル**。データ層(Phase 1)+ リサーチ層(Phase 2a)+ 評価エンジン
> (Phase 2b-1: walk-forward backtest)に加え、**モデル層(Tier0 ベースライン / Tier1 linear /
> Tier2 tree、共通 fit/predict、fold ごとに学習し OOS 評価)**まで実装。税(NISA)・ポートフォリオ・
> ダッシュボード・Tier3-4(deep/foundation)は次フェーズ([下記ロードマップ](#ロードマップ))。
...
investment-research-platform/
  ...
  notebooks/ 01..13     (01-07 実走、08-13 は次フェーズのスタブ)
  src/irp/   data/ macro/ features/ signals/ labels/ backtest/ models/ utils/   ← 実装済み
             portfolio/ tax/ visualization/  ← スタブ
  tests/
```
...
- **Phase 2b-3(次)** — 日本税(NISA、after-tax ドラッグ)→ ポートフォリオ構築(risk parity /
  mean-variance / 制約)→ Plotly ダッシュボード → NB 08–13 実走 → 最終 HTML レポート(失敗戦略も
  提示)。Tier3-4(deep / time-series foundation)は任意の後続。
