...
> **状態: Phase 2b-3 — 可視化 & レポート**。データ → リサーチ(features/signals/labels)→
> 評価エンジン(walk-forward backtest)→ モデル(Tier0-2)に加え、**Plotly 図群 + オフライン
> 自己完結 HTML レポート**(`irp.visualization`、plotly.js を inline 埋め込み)まで実装。
> 残るはポートフォリオ構築・日本税(NISA)([下記ロードマップ](#ロードマップ))。
...
investment-research-platform/
  ...
  notebooks/ 01..13     (01-07・12-13 実走、08-11 は次フェーズのスタブ)
  src/irp/   data/ macro/ features/ signals/ labels/ backtest/ models/ visualization/ utils/  ← 実装済み
             portfolio/ tax/  ← スタブ
  tests/
```
...
- **Phase 2b-3b(次)** — ポートフォリオ構築(risk parity / mean-variance / 制約)+ 日本税
  (NISA、after-tax ドラッグ)。NB 10–11 実走。Tier3-4(deep / time-series foundation、NB 08–09)は
  任意の後続。
