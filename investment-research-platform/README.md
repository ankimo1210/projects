...
> **状態: コア完成(Phase 2b 完了)**。データ → リサーチ(features/signals/labels)→ 評価エンジン
> (walk-forward backtest)→ モデル(Tier0-2)→ ポートフォリオ → 可視化/HTML → **日本税(NISA
> after-tax)** までエンドツーエンドで実装。任意の後続は Tier3-4(deep / time-series foundation)のみ
> ([下記ロードマップ](#ロードマップ))。
...
investment-research-platform/
  ...
  notebooks/ 01..13     (01-07・10-13 実走、08-09 は任意後続=Tier3-4 のスタブ)
...
- **Phase 2b-3c(完了)** — 日本税(`irp.tax`):年次 mark-to-market の譲渡益課税 + 3年損失繰越、
  課税口座 vs **NISA**(非課税)の after-tax 比較、`compare_accounts`。NB 11 実走。
- **任意の後続** — Tier3-4(deep learning / time-series foundation models、NB 08–09)。重い依存
  (torch 等)を伴うため、必要になったら追加する。コアの研究ループは完成済み。
