...
Skipped by default; run with ``IRP_LIVE=1`` (and network access):

    IRP_LIVE=1 uv run pytest investment-research-platform/tests/test_live.py -q
...
pytestmark = pytest.mark.skipif(
    not os.environ.get("IRP_LIVE"), reason="live network test; set IRP_LIVE=1 to run"
)
...
def test_live_sec_edgar_point_in_time():
    ...
    c = SecEdgarConnector(user_agent="irp-research live test (kikeuchi1210@gmail.com)")
...
def test_live_us_treasury_yields():
    from irp.macro import TreasuryConnector
...
def test_live_fred_point_in_time():
    from irp.macro import FredConnector, as_of
