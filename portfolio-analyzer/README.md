...
`--email <宛先>`（複数指定可）でサマリーを送ります。本文は HTML とプレーンテキストの両方。図は
**ダッシュボードの時系列セクションをヘッドレス Chromium で撮った PNG** を `cid:` の inline 画像として
本文に置きます（`chartshot.py`）。NAV と累計入金・累計損益・日次損益・決済済み・銘柄ごとの株価と
損益カードが、画面で見るのと同じレイアウトで入ります。`--png` は同じ画像を
`dist/pl-daily/pl-<基準日>.png` に書くだけのオプションです。

ブラウザが無い環境や `--no-image` のときは、代わりに**テーブルのセルで描いた図**（`emailchart.py`）を
本文に入れます。累計損益は折れ線＋ゼロ線からの塗り（左に高値・0・安値の軸、下に四半期の目盛り）、
日次損益は棒、保有表には銘柄ごとに 1 年の株価スパークライン。メールクライアントは `<script>` と
inline SVG を落とすので、これがスクリプト無しでも必ず出せる形です。
