...
ブラウザが無い環境や `--no-image` のときは、代わりに**テーブルのセルで描いた図**（`emailchart.py`：
累計損益・日次損益・銘柄別 1 年騰落率）を本文に入れます。メールクライアントは `<script>` と inline SVG を
落とすので、これがスクリプト無しでも必ず出せる形です。

実測で踏んだ制約を 2 つ記録しておきます。

- **Gmail の Web/API から送ると `<img>` タグごと削られます。** `cid:` で参照しても Content-ID が
  `ii_...` に書き換わり、画像は添付に回ります。inline 画像を本文に出せるのは SMTP 直送（このスクリプト）
  だけです。
- **同じ経路は CSS の `background` も全部削ります**（`bgcolor` 属性は残る）。テーブルで描く図の色は
  必ず属性で持たせます。
