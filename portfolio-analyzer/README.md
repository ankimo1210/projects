...
- [AI投資・利益の質・キオクシアの評価（2026-09-22）](docs/2026-09-22-ai-investment-research.md)

公開資料に基づく論点と検証方法を記録しています。個人の保有額・配分を含むレポートは
`data/*.private.*` に保存し、Git管理の調査文書とは分けます。
...
- **一般データ**: NAV・累計入金と累計PLの主要時系列、主要リスク（年率ボラ・1日95% VaR・外貨比率・限度超過件数）、リスク寄与上位2銘柄。詳細へのリンクに続き、口座別、資産配分、損益の内訳（含み／実現／配当／費用／為替換算／為替取引 ＝ NAV − 入金、
  下段に口座別の合計）
...
HTML冒頭には基準日より古い価格の注意を表示。一般データの主要時系列・リスクは詳細と同じ計算値を使います。メール画像生成用のレイアウトではサマリーを重複描画せず、既存の画像構成を維持します。
...
- **ルックスルー**: 商品ごとの `exposures`（セクター・発行体・資産）に評価額を掛けて集計。国は発行体の `country`、
  ...
  **推定**（外国株式を米 70／欧 20／他 10、外国債券を米 45／欧 40／他 15 で按分。`mix_note` に根拠）。
  ...
  表示の「国・地域」は国と地域を 1 つの分類（日本／米国／欧州／その他先進国／新興国、MSCI の区分で台湾・韓国は
  新興国）に寄せたもの（`risk.REGION_OF`）。発行体の国（例: TSMC＝台湾）と DC の地域按分（欧州・その他）が
  同じ表で混ざらないようにするためで、表に無い国は「その他」に入ります。
- **集中度**: 最大ポジション・上位 5・実効数（1/HHI。銘柄は口座をまたいで合算し、セクターは株式部分だけ）・最大セクター・ルックスルー後の
  最大銘柄・外貨比率・海外の最大単一国（国として分かる行だけ。地域でしか分からない按分は数えない）・現金比率。
  すべて総資産比。
- **リスク量**: 直近 252 営業日、**現在ウェイト固定**の日次円建て収益率（USD 建ては USD/JPY 込み）から、
  ...
  相関・ベータは薄まる方向に出ます（1329 と SMH の日次相関 0.22 に対し週次 0.62、2026-09 時点の実測）。
  **ベータ USD/JPY は為替感応度ではありません**。同じ日付どうしの回帰の傾きで、東証の引けと為替の日足の
  区切りのずれも入ります（日次 0.08 / 週次 0.24、標準誤差 ±0.1 前後）。株価が動かないときの感応度は外貨比率
  そのものなので、行の下に「円安 1% で +0.32%（株価一定）」を併記しています。
- **リスク寄与**: `RC_i = w_i (Σw)_i / wᵀΣw`（同じ窓の標本共分散、合計 1）。通貨・セクター・国・地域へは各商品の
...
`no headless browser found`）。それ以降は、ブラウザが無くてセル描画に落ちた回のメールの先頭（HTML・テキストとも）に
「このメールの注意」を出します。インストーラーが止まるときは、表示されるダウンロード URL の zip を手で展開します:
...
- **文字の大きさと幅**: 見出しは 15px・先頭の数字は 17px（Gmail 本体の文字が 14px 前後なので、それを
  大きく超えない）。件名が日付と版を出すので、受信一覧のプレビュー行には隠しの 1 行（総資産・日次・含み・限度超過）を
  入れています。本文は 600px 幅の 1 列で、どのブロックも最小幅 336px（幅 360px の画面から余白を引いた幅）以内に
  収めます。はみ出すと Gmail アプリはメール全体を縮小して表示するためで、7 列だった保有表（最小 594px）は
  固定幅 4 ブロックにして、狭い画面では 2×2 に折り返します。確認はヘッドレス Chromium で、ウィンドウ幅は
  500px 未満にならないので、幅 375px の iframe に入れて撮ります。
...
1. **洗い替えた明細は全て `estimated` になります。** 口座画面で確認した値ではなく
   ...
   評価額の精度が落ちたわけではありません
2. **時価の無いものは据え置きです。** 円現金（相場では動かないので `exact` のまま）と
   残高調整は元の日付の値のままで、理由は `source_note` に残ります。DC口座のファンドは日次レポートと
   同じく、運用会社の基準価額（`--as-of` 以前の最新）× 保有口数（`data/dc-holding.private.json` の残高に
   `data/dc-transactions.private.tsv` の掛金を足した万口）で洗い替え、口座損益は評価額 − 拠出金累計に
   します。基準価額の CSV が取れないときだけ据え置きます
