...
発行体ルックスルーに `theme` を付けると、商品をまたいで同じ材料に賭けている合計額を
集計します。指数やETFに分散して持っていても、動く材料が同じなら1つの賭けだからです。

```json
{"group": "issuer", "category": "Nvidia", "weight": 0.2201, "theme": "AIサプライチェーン"}
```

発行体を開示している部分だけの集計なので、**表示される比率は下限値**です
（カバー率は「発行体カバー率」カードを併読してください）。
