...
資金加重リターンと最大DD）・口座別・資産配分・損益の内訳（期間内と開設来）・保有・決済済み・
NAV と累計入金・累計損益・期間全体の日次損益・銘柄ごとのカード（数量／評価／比率／平均取得／
1W・1M・1Y／含み率、1 年の株価に平均取得の線と売買の帯、損益の推移）・注記。

図は**テーブルのセルで描きます**（`emailchart.py`）。メールクライアントは `<script>` と inline SVG を
落とすので、これがスクリプト無しでも必ず出せる形です。累計損益は折れ線＋ゼロ線からの塗り、NAV は
累計入金の線を重ね、銘柄カードの株価は高値・安値の軸つき。本文は 400 KB 台になり、Gmail では途中から
「メッセージ全体を表示」に折りたたまれます（下記）。

`--image` を付けると、図と銘柄カードの代わりに**ダッシュボードの時系列セクションをヘッドレス Chromium で
撮った PNG**（`chartshot.py`）を `cid:` の inline 画像として置きます。`--png` は同じ画像を
