...
発行時値による lag 補完は**各回号の先頭行だけ**に適用し、それ以外の欠損 lag は null のまま
保持する（学習行は `exposure_jpy` の非欠損を要求するため、欠損 lag 行が重みに混入しない）。
...
| `mortgage_rate_definition` | string | 使用した住宅ローン金利系列の定義ラベル。`mortgage_rate` mode の90%単一定義検査に使用 |
...
- `cashflows/<issue>`: 月初残高、約定元本、任意期限前償還、clean-up 元本、利息、月末残高。
  `--include-other-decrements` 時は scenario 名に `_totaldec` を付す。
- `*_summary.json`: WAL、最終元本期日、PV、dirty price / 100、Macaulay / effective duration、
  convexity、評価利回り、`published_decrements_included` と仮定した長期延滞・その他解約 SMM。
  非有限値は JSON では null として書き出す。
