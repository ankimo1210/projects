...
def parse_args():
    ...
    p.add_argument(
        "--skip-fetch", action="store_true", help="API取得をスキップしParquet→DBインポートのみ"
    )
    p.add_argument(
        "--no-clear", action="store_true", help="DB クリアをスキップして既存データに追記する"
    )
    return p.parse_args()
    ...
    def _sync_one(pref, year):
        ...
        if parquet.exists():
            ...
            if cnt > 0:
                with _log_lock:
                    print(
                        f"  → {PREF_NAMES.get(pref, pref)} {year}年: スキップ（Parquet既存 {cnt:,}件）",
                        flush=True,
                    )
        ...
        try:
            ...
            with _log_lock:
                print(
                    f"  ✗ {PREF_NAMES.get(pref, pref)} ({pref}) {year}年: {exc}  {elapsed:.0f}秒",
                    flush=True,
                )
        ...
        for pref in TARGET_PREFS:
            ...
            if p.exists():
                ...
                if not df_p.empty:
                    ...
                    imported += n
                    print(
                        f"  ✓ {PREF_NAMES.get(pref, pref)} {y}年: {len(df_p):,} 件 → DB", flush=True
                    )
...
def main():
    ...
    existing_years = [
        ...
        for y in all_years
        if any(
            (processed_dir / f"land_prices_y{y}_pref{p}_pc0.parquet").exists() for p in TARGET_PREFS
        )
    ]
    if args.no_clear:
