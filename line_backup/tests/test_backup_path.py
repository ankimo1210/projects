...
class TestCandidateDirs(unittest.TestCase):
    ...
    def test_appdata_candidate(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(
                any("Apple Computer" in p and "MobileSync" in p for p in paths_str)
            )
    ...
    def test_userprofile_candidate(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            paths_str = [str(p) for p in candidates]
            self.assertTrue(
                any("Apple" in p and "MobileSync" in p for p in paths_str)
            )
    ...
    def test_glob_wildcard_resolved(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            fake_pkg = Path(tmp) / "Packages" / "AppleInc.iTunes_abc123" / \
